...
if not DevRio:get(Server.."IdSARW") then 
...
DevRio:set(Server.."IdSARW",DevId) 
end ---ifok
else 
...
if not DevRio:get(Server.."TokenSARW") then 
...
DevRio:set(Server.."TokenSARW",TokenBot) 
end  
else 
...
Config = {
DevId = DevRio:get(Server.."IdSARW"),
TokenBot = DevRio:get(Server.."TokenSARW"),
SARW = DevRio:get(Server.."TokenSARW"):match("(%d+)"),
SudoIds = {DevRio:get(Server.."IdSARW")},
}
Create(Config, "./config.lua") 
file = io.open("SARW.sh", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/SARW
token="]]..DevRio:get(Server.."TokenSARW")..[["
while(true) do
rm -fr ../.telegram-cli
if [ ! -f ./tg ]; then
echo "┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉"
...
while(true) do
rm -fr ../.telegram-cli
screen -S SARW -X kill
screen -S SARW ./SARW.sh
done
]]) 
file:close() 
...
local Load_SARW = function() 
local f = io.open("./config.lua", "r") 
if not f then 
AutoSet() 
else 
f:close() 
DevRio:del(Server.."IdSARW");DevRio:del(Server.."TokenSARW")
end 
...
NameBot = (DevRio:get(SARW..'Rio:NameBot') or 'سرو')
--     Source SARW     --
...
local Status = DevRio:sismember(SARW..'Rio:RioSudo:',msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:SecondSudo:',msg.sender_user_id_) 
...
function Bot(msg) 
local var = false  
if msg.sender_user_id_ == tonumber(SARW) then  
var = true  
end  
return var  
end 
...
local Status = DevRio:sismember(SARW..'Rio:SudoBot:',msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:RioConstructor:'..msg.chat_id_,msg.sender_user_id_) 
if Status or RioSudo(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
----   Owner   ----
function Owner(msg) 
local Status = DevRio:sismember(SARW..'Rio:Owner:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:Constructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:Managers:'..msg.chat_id_,msg.sender_user_id_) 
...
----------  Admin  ---------
function Admin(msg) 
local Status = DevRio:sismember(SARW..'Rio:Admins:'..msg.chat_id_,msg.sender_user_id_) 
...
---------Vip Member---------
function VipMem(msg) 
local Status = DevRio:sismember(SARW..'Rio:VipMem:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:Cleaner:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:CleanerNum:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevRio:sismember(SARW..'Rio:CleanerMusic:'..msg.chat_id_,msg.sender_user_id_) 
if Status or SudoBot(msg) or Cleaner(msg) or RioConstructor(msg) or RioSudo(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
if DevRio:sismember(SARW..'Rio:Ban:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevRio:sismember(SARW..'Rio:BanAll:', user_id) then
var = true
else
var = false
end
...
if DevRio:sismember(SARW..'Rio:Muted:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevRio:sismember(SARW..'Rio:MuteAll:', user_id) then
var = true
else
var = false
end
...
--     Source SARW     --
function SARWFiles(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
plugin = dofile("Files/"..v)
if plugin.SARW and msg then
FilesText = plugin.SARW(msg)
end
end
end
...
if File_Name:lower():match('(%d+)') ~= SARW:lower() then 
...
local info_file = io.open('./'..SARW..'.json', "r"):read('*a')
local JsonInfo = JSON.decode(info_file)
vardump(JsonInfo)
DevRio:set(SARW.."Rio:NameBot",JsonInfo.BotName) 
for IdGps,v in pairs(JsonInfo.GroupsList) do
DevRio:sadd(SARW.."Rio:Groups",IdGps) 
DevRio:set(SARW.."Rio:Lock:Bots"..IdGps,"del") DevRio:hset(SARW.."Rio:Spam:Group:User"..IdGps ,"Spam:User","keed") 
...
DevRio:set(SARW..Lock..IdGps,true)
end
if v.RioConstructors then
for k,IdRioConstructors in pairs(v.RioConstructors) do
DevRio:sadd(SARW..'Rio:RioConstructor:'..IdGps,IdRioConstructors)  
print('تم رفع منشئين المجموعات')
end
end
if v.BasicConstructors then
for k,IdBasicConstructors in pairs(v.BasicConstructors) do
DevRio:sadd(SARW..'Rio:BasicConstructor:'..IdGps,IdBasicConstructors)  
print('تم رفع ( '..k..' ) منشئين اساسيين')
end
end
if v.Constructors then
for k,IdConstructors in pairs(v.Constructors) do
DevRio:sadd(SARW..'Rio:Constructor:'..IdGps,IdConstructors)  
print('تم رفع ( '..k..' ) منشئين')
end
end
if v.Managers then
for k,IdManagers in pairs(v.Managers) do
DevRio:sadd(SARW..'Rio:Managers:'..IdGps,IdManagers)  
...
vardump(IdAdmins)
DevRio:sadd(SARW..'Rio:Admins:'..IdGps,IdAdmins)  
print('تم رفع ( '..k..' ) ادمنيه')
end
end
if v.Vips then
for k,IdVips in pairs(v.Vips) do
DevRio:sadd(SARW..'Rio:VipMem:'..IdGps,IdVips)  
print('تم رفع ( '..k..' ) مميزين')
end
end
if v.LinkGroups then
if v.LinkGroups ~= "" then
DevRio:set(SARW.."Rio:Groups:Links"..IdGps,v.LinkGroups)   
print('( تم وضع روابط المجموعات )')
end
end
if v.Welcomes then
if v.Welcomes ~= "" then
DevRio:set(SARW.."Rio:Groups:Welcomes"..IdGps,v.Welcomes)   
...
--     Source SARW     --
local RioRank = function(msg) if SudoId(msg.sender_user_id_) then SARWTeam  = "المطور" elseif RioSudo(msg) then SARWTeam = "المطور" elseif SecondSudo(msg) then SARWTeam = "المطور" elseif SudoBot(msg) then SARWTeam = "المطور" elseif Manager(msg) then SARWTeam = "المدير" elseif Admin(msg) then SARWTeam = "الادمن" else SARWTeam = "العضو" end return SARWTeam end
function IdRank(user_id,chat_id) if tonumber(user_id) == tonumber(-1001582329762) then SARWTeam = 'مبرمج ' elseif tonumber(user_id) == tonumber(1922908800) then SARWTeam = 'مطور السورس' elseif tonumber(user_id) == tonumber(1922908800) then SARWTeam = 'مطور السورس' elseif tonumber(user_id) == tonumber(SARW) then SARWTeam = 'البوت' elseif SudoId(user_id) then SARWTeam = 'المطور الاساسي' elseif DevRio:sismember(SARW..'Rio:RioSudo:', user_id) then SARWTeam = 'المطور الاساسي' elseif DevRio:sismember(SARW..'Rio:SecondSudo:', user_id) then SARWTeam = 'المطور الاساسي²' elseif DevRio:sismember(SARW..'Rio:SudoBot:', user_id) then SARWTeam = DevRio:get(SARW.."Rio:SudoBot:Rd"..chat_id) or 'المطور' elseif DevRio:sismember(SARW..'Rio:Owner:'..chat_id, user_id) then SARWTeam = DevRio:get(SARW.."Rio:Owner:Rd"..chat_id) or 'المالك' elseif DevRio:sismember(SARW..'Rio:RioConstructor:'..chat_id, user_id) then SARWTeam = 'المالك' elseif DevRio:sismember(SARW..'Rio:BasicConstructor:'..chat_id, user_id) then SARWTeam = DevRio:get(SARW.."Rio:BasicConstructor:Rd"..chat_id) or 'المنشئ الاساسي' elseif DevRio:sismember(SARW..'Rio:Constructor:'..chat_id, user_id) then SARWTeam = DevRio:get(SARW.."Rio:Constructor:Rd"..chat_id) or 'المنشئ' elseif DevRio:sismember(SARW..'Rio:Managers:'..chat_id, user_id) then SARWTeam = DevRio:get(SARW.."Rio:Managers:Rd"..chat_id) or 'المدير' elseif DevRio:sismember(SARW..'Rio:Admins:'..chat_id, user_id) then SARWTeam = DevRio:get(SARW.."Rio:Admins:Rd"..chat_id) or 'الادمن' elseif DevRio:sismember(SARW..'Rio:VipMem:'..chat_id, user_id) then  SARWTeam = DevRio:get(SARW.."Rio:VipMem:Rd"..chat_id) or 'المميز' elseif DevRio:sismember(SARW..'Rio:Cleaner:'..chat_id, user_id) then  SARWTeam = DevRio:get(SARW.."Rio:Cleaner:Rd"..chat_id) or 'المنظف' else SARWTeam = DevRio:get(SARW.."Rio:mem:Rd"..chat_id) or 'العضو' end return SARWTeam end
--     Source SARW     --
function RankChecking(user_id,chat_id)
if SudoId(user_id) then
var = true  
elseif tonumber(user_id) == tonumber(SARW) then  
var = true
elseif DevRio:sismember(SARW..'Rio:RioSudo:', user_id) then
var = true
elseif DevRio:sismember(SARW..'Rio:SecondSudo:', user_id) then
var = true  
elseif DevRio:sismember(SARW..'Rio:SudoBot:', user_id) then
var = true 
elseif DevRio:sismember(SARW..'Rio:RioConstructor:'..chat_id, user_id) then
var = true
elseif DevRio:sismember(SARW..'Rio:Owner:'..chat_id, user_id) then
var = true
elseif DevRio:sismember(SARW..'Rio:BasicConstructor:'..chat_id, user_id) then
var = true
elseif DevRio:sismember(SARW..'Rio:Constructor:'..chat_id, user_id) then
var = true  
elseif DevRio:sismember(SARW..'Rio:Managers:'..chat_id, user_id) then
var = true  
elseif DevRio:sismember(SARW..'Rio:Admins:'..chat_id, user_id) then
var = true  
elseif DevRio:sismember(SARW..'Rio:VipMem:'..chat_id, user_id) then  
var = true 
else  
var = false
end  
...
local rio = (SARW..'Rio:Filters:'..msg.chat_id_)
if rio then
...
--     Source SARW     --
function riomoned(chat_id, user_id, msg_id, text, offset, length) local tt = DevRio:get(SARW..'endmsg') or '' tdcli_function ({ ID = "SendMessage", chat_id_ = chat_id, reply_to_message_id_ = msg_id, disable_notification_ = 0, from_background_ = 1, reply_markup_ = nil, input_message_content_ = { ID = "InputMessageText", text_ = text..'\n\n'..tt, disable_web_page_preview_ = 1, clear_draft_ = 0, entities_ = {[0]={ ID="MessageEntityMentionName", offset_=offset, length_=length, user_id_=user_id }, }, }, }, dl_cb, nil) end
--     Source SARW     --
function ChCheck(msg)
local var = true 
if DevRio:get(SARW.."Rio:ChId") then
local url , res = https.request('https://api.telegram.org/bot'..TokenBot..'/getchatmember?chat_id='..DevRio:get(SARW..'Rio:ChId')..'&user_id='..msg.sender_user_id_)
local data = json:decode(url)
if res ~= 200 or data.result.status == "left" or data.result.status == "kicked" then
var = false 
if DevRio:get(SARW..'Rio:ChText') then
local ChText = DevRio:get(SARW..'Rio:ChText')
send(msg.chat_id_,msg.id_,'['..ChText..']')
else
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevRio:get(SARW.."Rio:ChId"))
local GetInfo = JSON.decode(Check)
...
if DataText == '/delyes' and DevRio:get(SARW..'yes'..data.sender_user_id_) == 'delyes' then
DevRio:del(SARW..'yes'..data.sender_user_id_, 'delyes')
DevRio:del(SARW..'no'..data.sender_user_id_, 'delno')
...
if DataText == '/delno' and DevRio:get(SARW..'no'..data.sender_user_id_) == 'delno' then
DevRio:del(SARW..'yes'..data.sender_user_id_, 'delyes')
DevRio:del(SARW..'no'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم الغاء امر اطردني") 
end
--     Source SARW     --
if DataText == '/yesdel' and DevRio:get(SARW..'yesdel'..data.sender_user_id_) == 'delyes' then
DevRio:del(SARW..'yesdel'..data.sender_user_id_, 'delyes')
DevRio:del(SARW..'nodel'..data.sender_user_id_, 'delno')
if DevRio:sismember(SARW..'Rio:Constructor:'..data.chat_id_, data.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevRio:sismember(SARW..'Rio:Managers:'..data.chat_id_, data.sender_user_id_) then
Managers = 'المدراء • ' else Managers = '' end
if DevRio:sismember(SARW..'Rio:Admins:'..data.chat_id_, data.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevRio:sismember(SARW..'Rio:VipMem:'..data.chat_id_, data.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevRio:sismember(SARW..'Rio:Cleaner:'..data.chat_id_, data.sender_user_id_) then
cleaner = 'المنظفين • ' else cleaner = '' end
if DevRio:sismember(SARW..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
donky = 'المطايه • ' else donky = '' end
if DevRio:sismember(SARW..'Rio:Constructor:'..data.chat_id_, data.sender_user_id_) or DevRio:sismember(SARW..'Rio:Managers:'..data.chat_id_, data.sender_user_id_) or DevRio:sismember(SARW..'Rio:Admins:'..data.chat_id_, data.sender_user_id_) or DevRio:sismember(SARW..'Rio:VipMem:'..data.chat_id_, data.sender_user_id_) or DevRio:sismember(SARW..'Rio:Cleaner:'..data.chat_id_, data.sender_user_id_) or DevRio:sismember(SARW..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
DevRio:srem(SARW..'Rio:Constructor:'..data.chat_id_,data.sender_user_id_)
DevRio:srem(SARW..'Rio:Managers:'..data.chat_id_,data.sender_user_id_)
DevRio:srem(SARW..'Rio:Admins:'..data.chat_id_,data.sender_user_id_)
DevRio:srem(SARW..'Rio:VipMem:'..data.chat_id_,data.sender_user_id_)
DevRio:srem(SARW..'Rio:Cleaner:'..data.chat_id_,data.sender_user_id_)
DevRio:srem(SARW..'User:Donky:'..data.chat_id_,data.sender_user_id_)
...
--     Source SARW     --
if DevRio:get(SARW.."Rio:NewDev"..data.sender_user_id_) then
if DataText == '/setno' then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم الغاء امر تغير المطور الاساسي") 
DevRio:del(SARW.."Rio:NewDev"..data.sender_user_id_)
return false
end
if DataText == '/setyes' then
local NewDev = DevRio:get(SARW.."Rio:NewDev"..data.sender_user_id_)
...
if DataText == '/nodel' and DevRio:get(SARW..'nodel'..data.sender_user_id_) == 'delno' then
DevRio:del(SARW..'yesdel'..data.sender_user_id_, 'delyes')
DevRio:del(SARW..'nodel'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم الغاء امر نزلني") 
end
if DataText == '/YesRolet' and DevRio:get(SARW.."Rio:WittingStartRolet"..data.chat_id_..data.sender_user_id_) then
local List = DevRio:smembers(SARW..'Rio:ListRolet'..data.chat_id_) 
local UserName = List[math.random(#List)]
tdcli_function ({ID="SearchPublicChat",username_ = UserName},function(arg,dp) 
DevRio:incrby(SARW..'Rio:GamesNumber'..data.chat_id_..dp.id_, 5) 
end,nil) 
DevRio:del(SARW..'Rio:ListRolet'..data.chat_id_) 
DevRio:del(SARW.."Rio:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
EditMsg(Chat_Id2, Msg_Id2, "᥀︙*صاحب الحظ* ↫ ["..UserName.."]\n᥀︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*")
end
if DataText == '/NoRolet' then
DevRio:del(SARW..'Rio:ListRolet'..data.chat_id_) 
DevRio:del(SARW.."Rio:NumRolet"..data.chat_id_..data.sender_user_id_) 
DevRio:del(SARW.."Rio:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
...
if DevRio:sismember(SARW..'Rio:Tkeed:'..Chat_Id2, data.sender_user_id_) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..Chat_Id2.."&user_id="..data.sender_user_id_.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
DevRio:srem(SARW..'Rio:Tkeed:'..Chat_Id2, data.sender_user_id_)
...
DeleteMessage(Chat_Id2,{[0] = MsgId2})
https.request('https://ccccxcc.ml/SARW/Yt.php?Put=Photo&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&UserId='..data.sender_user_id_..'&Url='..Url..'&Name='..DirName)
...
https.request("https://api.telegram.org/bot"..TokenBot..'/answercallbackquery?callback_query_id='..data.id_..'&text='..URL.escape("᥀ جار تحميل النتائج يرجى الانتظار .. .")..'&show_alert=true')
https.request('https://ccccxcc.ml/SARW/Yt.php?Put=Video&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&Url='..Url..'&Name='..DirName)
...
https.request("https://api.telegram.org/bot"..TokenBot..'/answercallbackquery?callback_query_id='..data.id_..'&text='..URL.escape("᥀ جار تحميل النتائج يرجى الانتظار .. .")..'&show_alert=true')
https.request('https://ccccxcc.ml/SARW/Yt.php?Put=Audio&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&Url='..Url..'&Name='..DirName)
...
https.request("https://api.telegram.org/bot"..TokenBot..'/answercallbackquery?callback_query_id='..data.id_..'&text='..URL.escape("᥀ جار تحميل النتائج يرجى الانتظار .. .")..'&show_alert=true')
DeleteMessage(Chat_Id2,{[0] = MsgId2})
https.request('https://ccccxcc.ml/SARW/Yt.php?Put=Voice&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&Url='..Url..'&Name='..DirName)
end
if DataText and DataText:match('/DelRed:'..tonumber(data.sender_user_id_)..'(.*)') then
local Rio = DataText:match('/DelRed:'..tonumber(data.sender_user_id_)..'(.*)')
EditMsg(Chat_Id2, Msg_Id2, "᥀︙الكلمه ↫ "..Rio.." تم حذفها") 
DevRio:del(SARW..'Rio:Text:GpTexts'..Rio..data.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRedod'..data.chat_id_,Rio)
end
if DataText and DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)') then
local Rio = DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)')
local List = DevRio:smembers(SARW..'Rio:Text:GpTexts'..Rio..data.chat_id_)
if DevRio:get(SARW..'Rio:Add:GpRedod'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم انهاء وحفظ ↫ "..#List.." من الردود المتعدده للامر ↫ "..Rio) 
DevRio:del(SARW..'Rio:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
else
...
DevRio:del(SARW..'Rio:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
DevRio:del(SARW..'Rio:Text:GpTexts'..Rio..data.chat_id_)
DevRio:del(SARW..'Rio:Add:GpTexts'..data.sender_user_id_..data.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRedod'..data.chat_id_,Rio)
else
EditMsg(Chat_Id2, Msg_Id2, "᥀︙عذرا صلاحية الامر منتهيه !") 
end
...
DevRio:del(SARW..'Rio:Text:KtTexts'..Rio..data.chat_id_)
DevRio:srem(SARW..'Rio:Sudo:Kt'..data.chat_id_,Rio)
end
...
local List = DevRio:smembers(SARW..'Rio:Text:KtTexts'..Rio..data.chat_id_)
if DevRio:get(SARW..'Rio:Add:Kt'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم انهاء وحفظ ↫ "..#List.." من الاسئله للامر ↫ "..Rio) 
DevRio:del(SARW..'Rio:Add:Kt'..data.sender_user_id_..data.chat_id_)
else
EditMsg(Chat_Id2, Msg_Id2, "᥀︙عذرا صلاحية الامر منتهيه !") 
end
end
...
DevRio:del(SARW..'Rio:Add:Kt'..data.sender_user_id_..data.chat_id_)
DevRio:del(SARW..'Rio:Text:KtTexts'..Rio..data.chat_id_)
DevRio:del(SARW..'Rio:Add:KtTexts'..data.sender_user_id_..data.chat_id_)
DevRio:srem(SARW..'Rio:Sudo:Kt'..data.chat_id_,Rio)
else
EditMsg(Chat_Id2, Msg_Id2, "᥀︙عذرا صلاحية الامر منتهيه !") 
end
...
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/Song:"..data.sender_user_id_}},{{text=". TeAm SARW ᥀",url="t.me/YY55c"}}} 
...
local Text ='*᥀︙تم اختيار المقطع الصوتي لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/mp3:"..data.sender_user_id_}},{{text=". TeAm SARW ᥀",url="t.me/YY55c"}}} 
...
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/memz:"..data.sender_user_id_}},{{text=". TeAm SARW ᥀",url="t.me/YY55c"}}} 
...
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/gif:"..data.sender_user_id_}},{{text=". TeAm SARW ᥀",url="t.me/YY55c"}}} 
...
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/photo:"..data.sender_user_id_}},{{text=". TeAm SARW ᥀",url="t.me/YY55c"}}} 
...
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/anime:"..data.sender_user_id_}},{{text=". TeAm SARW ᥀",url="t.me/YY55c"}}} 
...
DevRio:sadd(SARW..'Rio:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:RioConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:SecondSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:sadd(SARW..'Rio:RioSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:RioConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:SecondSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:RioSudo:'..data.chat_id_,dp.id_)
...
DevRio:sadd(SARW..'Rio:Ban:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'᥀︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'YY55c')..')\n᥀︙تم حظره')
end
end
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:Ban:'..data.chat_id_,dp.id_)
...
if DevRio:sismember(SARW..'Rio:Muted:'..data.chat_id_, dp.id_) then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevRio:sadd(SARW..'Rio:Muted:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'᥀︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'YY55c')..')\n᥀︙تم كتمه')
end
end
end
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:Muted:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'᥀︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'YY55c')..')\n᥀︙تم الغاء كتمه')
end,nil)
end
...
tdcli_function ({ID = "GetUser",user_id_ = RioId},function(arg,dp) 
DevRio:srem(SARW..'Rio:BanAll:', dp.id_)
DevRio:srem(SARW..'Rio:MuteAll:', dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'᥀︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'YY55c')..')\n᥀︙تم الغاء العام')
end,nil)
end
...
DevRio:sadd(SARW..'Rio:Tkeed:'..data.chat_id_, dp.id_)
...
DevRio:srem(SARW..'Rio:Tkeed:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'᥀︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'YY55c')..')\n᥀︙تم الغاء تقيده')
end,nil)
end
--     Source SARW     --
...
local Del = DevRio:get(SARW..'Rio:Del')
local Text = [[
...
DevRio:del(SARW..'Rio:SecondSudo:')
...
DevRio:del(SARW..'Rio:Owner:'..data.chat_id_)
...
DevRio:del(SARW..'Rio:UsersMsgs'..data.chat_id_..':'..data.sender_user_id_)
...
DevRio:del(SARW..'Rio:Constructor:'..data.chat_id_)
...
DevRio:del(SARW..'Rio:Managers:'..data.chat_id_)
Text = "*᥀︙تم حذف المدراء*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• القائمه الرئيسيه •",callback_data="/DelList:"..data.sender_user_id_}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:del(SARW..'Rio:Admins:'..data.chat_id_)
...
DevRio:del(SARW..'Rio:VipMem:'..data.chat_id_)
...
DevRio:del(SARW..'Rio:Cleaner:'..data.chat_id_)
...
DevRio:del(SARW..'Rio:Muted:'..data.chat_id_)
...
local List = DevRio:smembers(SARW..'Rio:Tkeed:'..data.chat_id_)
for k,v in pairs(List) do   
...
DevRio:srem(SARW..'Rio:Tkeed:'..data.chat_id_, v)
end
...
DevRio:del(SARW..'Rio:Ban:'..data.chat_id_)
...
DevRio:del(SARW..'Rio:BanAll:')
DevRio:del(SARW..'Rio:MuteAll:')
...
--     Source SARW     --
if DataText and DataText:match('/DelGpRed:'..tonumber(data.sender_user_id_)..'(.*)') and not DevRio:get(SARW..'Rio:Lock:GpRd'..data.chat_id_) then
local Rio = DataText:match('/DelGpRed:'..tonumber(data.sender_user_id_)..'(.*)')
local redod = DevRio:smembers(SARW..'Rio:Manager:GpRed'..data.chat_id_)
if #redod == 0 then
Text = "*᥀︙لا توجد ردود مضافه*"
else
for k,v in pairs(redod) do
DevRio:del(SARW..'Rio:Gif:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Voice:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Audio:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Photo:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Stecker:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Video:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:File:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Text:GpRed'..v..data.chat_id_)
DevRio:del(SARW..'Rio:Manager:GpRed'..data.chat_id_)
end
...
if DataText and DataText:match('/DelGpRedAll:'..tonumber(data.sender_user_id_)..'(.*)') and not DevRio:get(SARW..'Rio:Lock:GpRd'..data.chat_id_) then
local Rio = DataText:match('/DelGpRedAll:'..tonumber(data.sender_user_id_)..'(.*)')
local redod = DevRio:smembers(SARW.."Rio:Sudo:AllRed")
if #redod == 0 then
Text = "*᥀︙لا توجد ردود مضافه*"
else
for k,v in pairs(redod) do
DevRio:del(SARW.."Rio:Add:AllRed"..v)
DevRio:del(SARW.."Rio:Gif:AllRed"..v)
DevRio:del(SARW.."Rio:Voice:AllRed"..v)
DevRio:del(SARW.."Rio:Audio:AllRed"..v)
DevRio:del(SARW.."Rio:Photo:AllRed"..v)
DevRio:del(SARW.."Rio:Stecker:AllRed"..v)
DevRio:del(SARW.."Rio:Video:AllRed"..v)
DevRio:del(SARW.."Rio:File:AllRed"..v)
DevRio:del(SARW.."Rio:Text:AllRed"..v)
DevRio:del(SARW.."Rio:Sudo:AllRed")
end
...
local List = DevRio:smembers(SARW.."List:Cmd:Group:New"..data.chat_id_)
for k,v in pairs(List) do
DevRio:del(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":"..v)
DevRio:del(SARW.."List:Cmd:Group:New"..data.chat_id_)
end
...
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":ا","ايدي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"ا")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":م","رفع مميز")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"م")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":اد","رفع ادمن")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"اد")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":مد","رفع مدير")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"مد")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":من","رفع منشئ")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"من")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":اس","رفع منشئ اساسي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"اس")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":مط","رفع مطور")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"مط")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":ثانوي","رفع مطور ثانوي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"ثانوي")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":تك","تنزيل الكل")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"تك")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":تعط","تعطيل الايدي بالصوره")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"تعط")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":تفع","تفعيل الايدي بالصوره")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"تفع")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":ر","الرابط")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"ر")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":رر","ردود المدير")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"رر")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":،،","مسح المكتومين")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"،،")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":رد","اضف رد")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"رد")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":غ","غنيلي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"غ")
DevRio:set(SARW.."Set:Cmd:Group:New1"..data.chat_id_..":#","مسح قائمه العام")
DevRio:sadd(SARW.."List:Cmd:Group:New"..data.chat_id_,"#")
...
DevRio:sadd(SARW..'Rio:Admins:'..data.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevRio:srem(SARW..'Rio:Admins:'..data.chat_id_, admins[i].user_id_)
end
end,nil)
else
DevRio:sadd(SARW..'Rio:Admins:'..data.chat_id_, admins[i].user_id_)
...
DevRio:sadd(SARW.."Rio:BasicConstructor:"..data.chat_id_,admins[i].user_id_)
DevRio:sadd(SARW.."Rio:RioConstructor:"..data.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevRio:srem(SARW.."Rio:BasicConstructor:"..data.chat_id_,admins[i].user_id_)
DevRio:srem(SARW.."Rio:RioConstructor:"..data.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevRio:sismember(SARW..'Rio:Groups',data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, '᥀︙المجموعه بالتاكيد مفعله')
else
Text = "᥀︙تم تفعيل المجموعه "..dp.title_
keyboard = {} 
keyboard.inline_keyboard = {{{text="‹ ترتيب الاوامر ›",callback_data="/SetCmdGp:"..data.sender_user_id_},{text="‹ رفع الادمنيه ›",callback_data="/UploadAdmin:"..data.sender_user_id_}},{{text="‹ غادر ›",callback_data="/LeaveBot:"..data.sender_user_id_},{text="‹ تعطيل ›",callback_data="/Stop:"..data.sender_user_id_}},{{text="‹ TeAm SARW ›",url="t.me/YY55c"}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
DevRio:sadd(SARW.."Rio:Groups",data.chat_id_)
if not DevRio:get(SARW..'Rio:SudosGp'..data.sender_user_id_..data.chat_id_) and not SecondSudo(data) then 
DevRio:incrby(SARW..'Rio:Sudos'..data.sender_user_id_,1)
DevRio:set(SARW..'Rio:SudosGp'..data.sender_user_id_..data.chat_id_,"rio")
end
...
DevRio:set(SARW.."Rio:Groups:Links"..data.chat_id_,LinkGroup) 
...
tdcli_function ({ID = "GetUser",user_id_ = data.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=data.chat_id_},function(arg,dp) 
if not DevRio:sismember(SARW..'Rio:Groups',data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙المجموعه بالتاكيد معطله") 
else
Text = "᥀︙تم تعطيل المجموعه "..dp.title_
keyboard = {}
keyboard.inline_keyboard = {{{text="‹ غادر ›",callback_data="/LeaveBot:"..data.sender_user_id_},{text="‹ تفعيل ›",callback_data="/On:"..data.sender_user_id_}},{{text="‹ TeAm SARW ›",url="t.me/YY55c"}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
DevRio:srem(SARW.."Rio:Groups",data.chat_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
Text = "᥀︙تم ترتيب الاوامر بالشكل التالي ~\n᥀︙ ايدي - ا .\n᥀︙ رفع مميز - م .\n᥀︙رفع ادمن - اد .\n᥀︙ رفع مدير - مد . \n᥀︙ رفع منشى - من . \n᥀︙ رفع منشئ الاساسي - اس  .\n᥀︙ رفع مطور - مط .\n᥀︙رفع مطور ثانوي - ثانوي .\n᥀︙ تنزيل الكل - تك .\n᥀︙ تعطيل الايدي بالصوره - تعط .\n᥀︙ تفعيل الايدي بالصوره - تفع .\n᥀︙ الرابط - ر .\n᥀︙ ردود المدير - رر .\n᥀︙ مسح المكتومين - ،، .\n᥀︙ اضف رد - رد .\n᥀︙ مسح سحكاتي - سح .\n᥀︙ مسح رسائلي - رس .\n᥀︙ غنيلي - غ .\n᥀︙مسح قائمه العام"
keyboard = {} 
keyboard.inline_keyboard = {{{text="‹ رجوع ›",callback_data="/Reload:"..data.sender_user_id_}},{{text='‹ TeAm SARW ›',url="t.me/YY55c"}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
--     Source David     --
...
DevRio:sadd(SARW..'Rio:BasicConstructor:'..data.chat_id_,Manager_id)  
DevRio:sadd(SARW..'Rio:RioConstructor:'..data.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
local DelSudoRep = DevRio:get(SARW..'DelSudoRep')
DevRio:del(SARW.."Rio:Sudo:AllRed",DelSudoRep)
DevRio:del(SARW.."Rio:Add:AllText"..data.sender_user_id_)
DevRio:del(SARW.."Rio:Add:AllRed"..data.sender_user_id_)
DevRio:del(SARW.."DelSudoRep")
Text = "᥀︙تم الغاء حفظ الرد"
keyboard = {} 
keyboard.inline_keyboard = {{{text = '᥀ TeAm SARW .',url="t.me/YY55c"}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
return false
end
...
local DelManagerRep = DevRio:get(SARW..'DelManagerRep'..data.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRed'..data.chat_id_,DelManagerRep)
DevRio:del(SARW..'Rio:Add:GpText'..data.sender_user_id_..data.chat_id_)
DevRio:del(SARW..'Rio:Add:GpRed'..data.sender_user_id_..data.chat_id_)
DevRio:del(SARW..'DelManagerRep'..data.chat_id_)
Text = "᥀︙تم الغاء حفظ الرد"
keyboard = {} 
keyboard.inline_keyboard = {{{text = '᥀ TeAm SARW .',url="t.me/YY55c"}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Cmd = DevRio:get(SARW..'Rio:Cmd')
local Text = [[
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Msg = DevRio:get(SARW..'Rio:Msg')
local Text = [[
...
DevRio:del(SARW..'Rio:EditMsg'..data.chat_id_..data.sender_user_id_)
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Filter = DevRio:get(SARW..'Rio:Filter')
local Text = [[
...
DevRio:del(SARW.."Rio:FilterAnimation"..data.chat_id_)
...
DevRio:del(SARW.."Rio:FilterSteckr"..data.chat_id_)
...
DevRio:del(SARW.."Rio:FilterPhoto"..data.chat_id_)
...
DevRio:del(SARW..'Rio:Filters:'..data.chat_id_)
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local locklist = DevRio:get(SARW..'Rio:locklist')
local Text = [[
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local LockBotList = DevRio:get(SARW..'Rio:LockBotList')
local Text = [[
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local LockSpamList = DevRio:get(SARW..'Rio:LockSpamList')
local Text = [[
...
if not DevRio:get(SARW..'Rio:Lock:Join'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Join'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:Join'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Join'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:Text'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Text'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:Text'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Text'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:TagServr'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:TagServr'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:TagServr'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:TagServr'..data.chat_id_) 
...
if not DevRio:get(SARW..'Rio:Lock:EditMsgs'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:EditMsgs'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:EditMsgs'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:EditMsgs'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:Stickers'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Stickers'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:Stickers'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Stickers'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:Gifs'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Gifs'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:Gifs'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Gifs'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:Photo'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Photo'..data.chat_id_,true) 
...
if DevRio:get(SARW..'Rio:Lock:Photo'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Photo'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:Videos'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Videos'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:Videos'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Videos'..data.chat_id_)
...
if not DevRio:get(SARW..'Rio:Lock:Forwards'..data.chat_id_) then
DevRio:set(SARW..'Rio:Lock:Forwards'..data.chat_id_,true)
...
if DevRio:get(SARW..'Rio:Lock:Forwards'..data.chat_id_) then
DevRio:del(SARW..'Rio:Lock:Forwards'..data.chat_id_) 
...
DevRio:set(SARW.."Rio:Lock:Bots"..data.chat_id_,"kick")  
...
DevRio:set(SARW.."Rio:Lock:Bots"..data.chat_id_,"ked")   
...
DevRio:set(SARW.."Rio:Lock:Bots"..data.chat_id_,"del")  
...
DevRio:del(SARW.."Rio:Lock:Bots"..data.chat_id_) 
...
DevRio:hset(SARW.."Rio:Spam:Group:User"..data.chat_id_ ,"Spam:User","kick")  
...
DevRio:hset(SARW.."Rio:Spam:Group:User"..data.chat_id_ ,"Spam:User","keed")  
...
DevRio:hset(SARW.."Rio:Spam:Group:User"..data.chat_id_ ,"Spam:User","del")  
...
DevRio:hset(SARW.."Rio:Spam:Group:User"..data.chat_id_ ,"Spam:User","mute")  
...
DevRio:hdel(SARW.."Rio:Spam:Group:User"..data.chat_id_ ,"Spam:User")  
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Source = DevRio:get(SARW..'Rio:Source')
local Text = [[
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Media = DevRio:get(SARW..'Rio:Media')
local Text = [[
...
local List = DevRio:smembers(SARW.."Rio:cleaner"..data.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
local List = DevRio:smembers(SARW.."Rio:cleanermusic"..data.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevRio:del(SARW.."Rio:cleanermusic"..data.chat_id_)
else
...
if DevRio:get(SARW.."Rio:Left:Bot"..SARW) then
...
if not DevRio:get(SARW.."Rio:Lock:GpLinksinline"..data.chat_id_) then 
tdcli_function({ID ="GetChat",chat_id_=data.chat_id_},function(arg,ta) 
local linkgpp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..data.chat_id_)) or DevRio:get(SARW.."Private:Group:Link"..data.chat_id_) 
...
if not DevRio:get(SARW..'Rio:Lock:GpLinks'..data.chat_id_) then 
if DevRio:get(SARW.."Rio:Groups:Links"..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙Group Link ↬ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"..DevRio:get(SARW.."Rio:Groups:Links"..data.chat_id_), 1, "html")
else 
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local order = DevRio:get(SARW..'Rio:order')
local Text = [[
...
DevRio:set(SARW..'Rio:Lock:Id:Photo'..data.chat_id_,true)
...
DevRio:del(SARW..'Rio:Lock:Id:Photo'..data.chat_id_)
...
DevRio:set(SARW..'Rio:Lock:Id'..data.chat_id_,true)
...
DevRio:del(SARW..'Rio:Lock:Id'..data.chat_id_)
...
DevRio:set(SARW.."Rio:Lock:GpLinks"..data.chat_id_,"ok")
...
DevRio:del(SARW.."Rio:Lock:GpLinks"..data.chat_id_)
...
DevRio:set(SARW.."Rio:Lock:GpLinksinline"..data.chat_id_,"ok")
...
DevRio:del(SARW.."Rio:Lock:GpLinksinline"..data.chat_id_)
...
DevRio:del(SARW.."Rio:Lock:Welcome"..data.chat_id_)
...
DevRio:set(SARW.."Rio:Lock:Welcome"..data.chat_id_,true)
...
DevRio:set(SARW..'Rio:Lock:AllRed'..data.chat_id_,true)
...
DevRio:del(SARW..'Rio:Lock:AllRed'..data.chat_id_)
...
DevRio:set(SARW..'Rio:Lock:GpRed'..data.chat_id_,true)
...
DevRio:del(SARW..'Rio:Lock:GpRed'..data.chat_id_)
...
DevRio:set(SARW.."Rio:Del:Me"..data.chat_id_, true)
Text = "*᥀︙تم تعطيل نزلني*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:del(SARW.."Rio:Del:Me"..data.chat_id_)
Text = "*᥀︙تم تفعيل نزلني*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:set(SARW.."Rio:Kick:Me"..data.chat_id_, true)
...
DevRio:del(SARW.."Rio:Kick:Me"..data.chat_id_)
...
DevRio:set(SARW.."Rio:Lock:KickBan"..data.chat_id_,"true")
Text = "*᥀︙تم تعطيل الحظر*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:del(SARW.."Rio:Lock:KickBan"..data.chat_id_)
Text = "*᥀︙تم تفعيل الحظر*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:set(SARW.."Rio:Lock:ProSet"..data.chat_id_,"true")
Text = "*᥀︙تم تعطيل الرفع*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:del(SARW.."Rio:Lock:ProSet"..data.chat_id_)
Text = "*᥀︙تم تفعيل الرفع*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevRio:set(SARW..'Rio:Lock:Games'..data.chat_id_,true)  
...
DevRio:del(SARW..'Rio:Lock:Games'..data.chat_id_) 
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Help = DevRio:get(SARW..'Rio:Help')
local Text = [[
...
local Help = DevRio:get(SARW..'Rio:Help1')
local Text = [[
...
local Help = DevRio:get(SARW..'Rio:Help2')
local Text = [[
᥀︙اوامر الادمنيه ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙الاعدادت
...
local Help = DevRio:get(SARW..'Rio:Help3')
local Text = [[
᥀︙اوامر المدراء ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙فحص البوت
...
local Help = DevRio:get(SARW..'Rio:Help4')
local Text = [[
᥀︙اوامر المنشئين ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙تنزيل الكل
...
local Help = DevRio:get(SARW..'Rio:Help5')
local Text = [[
᥀︙اوامر المطورين ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙الكروبات
...
if tonumber(Rio) == tonumber(data.sender_user_id_) then
local Help = DevRio:get(SARW..'Rio:Help6')
local Text = [[
᥀︙اوامر الاعضاء ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
if text and DevRio:get(SARW.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = DevRio:get(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
DevRio:del(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
DevRio:del(SARW.."Set:Cmd:Group:New"..msg.chat_id_)
DevRio:srem(SARW.."List:Cmd:Group:New"..msg.chat_id_,text)
...
DevRio:del(SARW.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if text and text:match('^'..(DevRio:get(SARW..'Rio:NameBot') or "سرو")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(DevRio:get(SARW..'Rio:NameBot') or "سرو")..' ','')
end
if data.message_.content_.text_ then
local NewCmmd = DevRio:get(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":"..data.message_.content_.text_)
if NewCmmd then
data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
end
end
if text and DevRio:get(SARW.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
DevRio:set(SARW.."Set:Cmd:Group:New"..msg.chat_id_,text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙ارسل الامر الجديد", 1, 'html')
DevRio:del(SARW.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
DevRio:set(SARW.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_,"true1") 
return false
end
if text and DevRio:get(SARW.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
local NewCmd = DevRio:get(SARW.."Set:Cmd:Group:New"..msg.chat_id_)
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text,NewCmd)
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الامر", 1, 'html')
DevRio:del(SARW.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if Constructor(msg) then
if text == "الاوامر المضافه" and ChCheck(msg) then
local List = DevRio:smembers(SARW.."List:Cmd:Group:New"..msg.chat_id_.."") 
t = "᥀︙قائمة الاوامر المضافه ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
Cmds = DevRio:get(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
if Cmds then 
t = t..k.."~ ("..v..") • {"..Cmds.."}\n"
else
t = t..k.."~ ("..v..") \n"
...
local List = DevRio:smembers(SARW.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(List) do
DevRio:del(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
DevRio:del(SARW.."List:Cmd:Group:New"..msg.chat_id_)
end
...
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":ا","ايدي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"ا")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":م","رفع مميز")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"م")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":اد","رفع ادمن")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"اد")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":مد","رفع مدير")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"مد")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":من","رفع منشئ")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"من")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":اس","رفع منشئ اساسي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"اس")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":مط","رفع مطور")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"مط")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":ثانوي","رفع مطور ثانوي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"ثانوي")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":تك","تنزيل الكل")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"تك")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":تعط","تعطيل الايدي بالصوره")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"تعط")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":تفع","تفعيل الايدي بالصوره")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"تفع")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":ر","الرابط")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"ر")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":رر","ردود المدير")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"رر")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":،،","مسح المكتومين")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"،،")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":رد","اضف رد")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"رد")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":سح","مسح سحكاتي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"سح")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":رس","مسح رسائلي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"رس")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":غ","غنيلي")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"غ")
DevRio:set(SARW.."Set:Cmd:Group:New1"..msg.chat_id_..":#","مسح قائمه العام")
DevRio:sadd(SARW.."List:Cmd:Group:New"..msg.chat_id_,"#")
send(msg.chat_id_, msg.id_,"᥀︙تم ترتيب الاوامر بالشكل التالي ~\n᥀︙ ايدي - ا .\n᥀︙ رفع مميز - م .\n᥀︙رفع ادمن - اد .\n᥀︙ رفع مدير - مد . \n᥀︙ رفع منشى - من . \n᥀︙ رفع منشئ الاساسي - اس  .\n᥀︙ رفع مطور - مط .\n᥀︙رفع مطور ثانوي - ثانوي .\n᥀︙ تنزيل الكل - تك .\n᥀︙ تعطيل الايدي بالصوره - تعط .\n᥀︙ تفعيل الايدي بالصوره - تفع .\n᥀︙ الرابط - ر .\n᥀︙ ردود المدير - رر .\n᥀︙ مسح المكتومين - ،، .\n᥀︙ اضف رد - رد .\n᥀︙ مسح سحكاتي - سح .\n᥀︙ مسح رسائلي - رس .\n᥀︙ غنيلي - غ .\n᥀︙مسح قائمه العام")  
end
if text == "اضف امر" and ChCheck(msg) or text == "اضافة امر" and ChCheck(msg) or text == "اضافه امر" and ChCheck(msg) then
DevRio:set(SARW.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
DevRio:set(SARW.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
local List = DevRio:smembers(SARW.."Coomds"..msg.chat_id_)
if #List == 0 then
...
for k,v in pairs(List) do
var = DevRio:get(SARW.."Comd:New:rt:Rio:"..v..msg.chat_id_)
if var then
t = t..k.."~ "..v.." • ("..var..")\n"
else
t = t..k.."~ "..v.."\n"
...
local List = DevRio:smembers(SARW.."Coomds"..msg.chat_id_)
for k,v in pairs(List) do
DevRio:del(SARW.."Comd:New:rt:Rio:"..v..msg.chat_id_)
DevRio:del(SARW.."Coomds"..msg.chat_id_)
end
...
DevRio:set(SARW.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_,ComdNew)  
DevRio:sadd(SARW.."Coomds"..msg.chat_id_,ComdNew)  
DevRio:setex(SARW.."Comd:New"..msg.chat_id_..msg.sender_user_id_,200,true)  
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙ارسل نوع الصلاحيه \n{ عضو • مميز  • ادمن  • مدير }\n᥀︙ارسل الغاء لالغاء الامر ", 1, 'html')
end
if text and text:match("^حذف صلاحيه (.*)$") and ChCheck(msg) or text and text:match("^مسح صلاحيه (.*)$") and ChCheck(msg) then 
ComdNew = text:match("^حذف صلاحيه (.*)$") or text:match("^مسح صلاحيه (.*)$")
DevRio:del(SARW.."Comd:New:rt:Rio:"..ComdNew..msg.chat_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حذف الصلاحيه", 1, 'html')
end
if DevRio:get(SARW.."Comd:New"..msg.chat_id_..msg.sender_user_id_) then 
if text and text:match("^↫ الغاء ᥀$") then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر", 1, 'html')
DevRio:del(SARW.."Comd:New"..msg.chat_id_..msg.sender_user_id_) 
return false  
end 
if text == "مدير" then
...
local textn = DevRio:get(SARW.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_)  
DevRio:set(SARW.."Comd:New:rt:Rio:"..textn..msg.chat_id_,text)
...
if DevRio:sismember(SARW.."Coomds"..msg.chat_id_,DEV_RIO) then
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local mrrio = DevRio:get(SARW.."Comd:New:rt:Rio:"..DEV_RIO..msg.chat_id_)
...
DevRio:set(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_RIO) 
DevRio:sadd(SARW..'Rio:VipMem:'..msg.chat_id_, result.sender_user_id_)
elseif mrrio == "ادمن" and Admin(msg) then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'YY55c')..')'..' ❩\n᥀︙تم رفعه ❨ '..DEV_RIO..' ❩ بنجاح', 1, 'md')
DevRio:set(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_RIO)
DevRio:sadd(SARW..'Rio:Admins:'..msg.chat_id_, result.sender_user_id_)
elseif mrrio == "مدير" and Manager(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'YY55c')..')'..' ❩\n᥀︙تم رفعه ❨ '..DEV_RIO..' ❩ بنجاح', 1, 'md')
DevRio:set(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_RIO)  
DevRio:sadd(SARW..'Rio:Managers:'..msg.chat_id_, result.sender_user_id_)
...
DevRio:srem(SARW..'Rio:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevRio:del(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrrio == "ادمن" and Admin(msg) then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'YY55c')..')'..' ❩\n᥀︙تم تنزيله ❨ '..DEV_RIO..' ❩ بنجاح', 1, 'md')
DevRio:srem(SARW..'Rio:Admins:'..msg.chat_id_, result.sender_user_id_)
DevRio:del(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrrio == "مدير" and Manager(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'YY55c')..')'..' ❩\n᥀︙تم تنزيله ❨ '..DEV_RIO..' ❩ بنجاح', 1, 'md')
DevRio:srem(SARW..'Rio:Managers:'..msg.chat_id_, result.sender_user_id_)
DevRio:del(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrrio == "عضو" then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'YY55c')..')'..' ❩\n᥀︙تم تنزيله ❨ '..DEV_RIO..' ❩ بنجاح', 1, 'md')
end
...
if DevRio:sismember(SARW.."Coomds"..msg.chat_id_,text1[2]) then
function py_username(extra, result, success)   
if result.id_ then
local mrrio = DevRio:get(SARW.."Comd:New:rt:Rio:"..text1[2]..msg.chat_id_)
if mrrio == "مميز" and VipMem(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevRio:sadd(SARW..'Rio:VipMem:'..msg.chat_id_, result.id_)
DevRio:set(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrrio == "ادمن" and Admin(msg) then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevRio:sadd(SARW..'Rio:Admins:'..msg.chat_id_, result.id_)
DevRio:set(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrrio == "مدير" and Manager(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevRio:sadd(SARW..'Rio:Managers:'..msg.chat_id_, result.id_)
DevRio:set(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrrio == "عضو" then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevRio:srem(SARW..'Rio:VipMem:'..msg.chat_id_, result.id_)
DevRio:del(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrrio == "ادمن" and Admin(msg) then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevRio:srem(SARW..'Rio:Admins:'..msg.chat_id_, result.id_)
DevRio:del(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrrio == "مدير" and Manager(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevRio:srem(SARW..'Rio:Managers:'..msg.chat_id_, result.id_)
DevRio:del(SARW.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrrio == "عضو" then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'YY55c')..')'..' ❩\n᥀︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevRio:incr(SARW..'Rio:UsersMsgs'..SARW..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_)
DevRio:incr(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
DevRio:incr(SARW..'Rio:MsgNumberDay'..msg.chat_id_..':'..os.date('%d'))  
ChatType = 'sp' 
elseif id:match("^(%d+)") then
if not DevRio:sismember(SARW.."Rio:Users",msg.chat_id_) then
DevRio:sadd(SARW.."Rio:Users",msg.chat_id_)
end
ChatType = 'pv' 
else
ChatType = 'gp' 
...
if not DevRio:get(SARW..'Rio:Start:Time'..msg.sender_user_id_) then
tdcli_function({ID="GetUser",user_id_=DevId},function(arg,dp)
tdcli_function({ID="GetUser",user_id_=SARW},function(arg,dpbot)
local inline = {
{{text="᥀ المطور .",url="t.me/"..(dp.username_ or "YY55c")}},
{{text="᥀ السورس .",url="https://t.me/YY55c"},{text="᥀ لتنصيب بوت .",url="https://t.me/lIHHHl"}},{{text="᥀ اضفني في مجموعتك .",url="t.me/"..dpbot.username_.."?startgroup=botstart"}}
}
local start = DevRio:get(SARW.."Rio:Start:Bot")
if start then 
Start_Source = start
else
...
DevRio:setex(SARW..'Rio:Start:Time'..msg.sender_user_id_,300,true)
return false
end 
--     Source SARW     --
if not SecondSudo(msg) and not DevRio:sismember(SARW..'Rio:Ban:Pv',msg.sender_user_id_) and not DevRio:get(SARW..'Rio:Texting:Pv') then
...
DevRio:sadd(SARW..'Rio:Ban:Pv',data.id_)  
return false  
end 
if text == 'الغاء الحظر' or text == 'الغاء حظر' then
local Text = '᥀︙العضو ↫ ['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..data.id_..')'..'\n᥀︙تم الغاء حظره من التواصل'
SendText(DevId,Text,msg.id_/2097152/0.5,'md') 
DevRio:srem(SARW..'Rio:Ban:Pv',data.id_)  
return false  
end 
...
--     Source SARW     --
if text and DevRio:get(SARW..'Rio:Start:Bots'..msg.sender_user_id_) then
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الستارت', 1, 'md')
DevRio:del(SARW..'Rio:Start:Bots'..msg.sender_user_id_) 
return false
end
DevRio:set(SARW.."Rio:Start:Bot",text)  
...
DevRio:set(SARW..'Rio:Start:Bots'..msg.sender_user_id_,true) 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل التواصل بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Texting:Pv') 
end
if text == 'تعطيل التواصل' and ChCheck(msg) or text == '↫ تعطيل التواصل ᥀' and ChCheck(msg) then  
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل التواصل بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Texting:Pv',true) 
end
if Sudo(msg) then
...
DevRio:del(SARW.."Rio:Lock:AutoFile")
end
...
DevRio:set(SARW.."Rio:Lock:AutoFile",true) 
end
end
end
--     Source SARW     --
...
local SARWTeam = {'- Nothing stops him who wants you .','make them wish they had you !.','Maybe a magical girl','٫ 𝖡𝖾 𝗌𝗂𝗆𝗉𝗅𝖾 𝖺𝗇𝖽 𝖽𝗂𝖿𝖿𝖾𝗋𝖾𝗇𝗍','. 𝖬𝖺𝗄𝖾 𝖽𝗋𝖾𝖺𝗆𝗌 𝖿𝗋𝗈𝗆 𝗒𝗈𝗎𝗋 𝗋𝖾𝖿𝗋𝖺𝖼𝗍𝗂𝗈𝗇𝗌 . .',':Life is lying .','𝖨 𝗐𝗂𝗅𝗅 𝖺𝗅𝗐𝖺𝗒𝗌 𝗅𝗈𝗏𝖾 𝗒𝗈𝗎 𝗇𝖾𝗏𝖾𝗋 𝖿𝗈𝗋𝗀𝖾𝗍'}  
Dev_Rio(msg.chat_id_, msg.id_, 1, ''..SARWTeam[math.random(#SARWTeam)]..'' , 1, 'md')  
return false
end
--     Source SARW     --
...
local DevText = DevRio:get(SARW.."DevText")
if DevRio:get(SARW.."Rio:ChId") then DevCh = '\n᥀︙*Dev Ch* ↬ [@'..DevCh1..']' else DevCh = '' end
if DevText then
...
local my_ph = DevRio:get(SARW.."Rio:Photo:Profile"..msg.chat_id_)
if not my_ph then
...
if not DevRio:get(SARW..'Rio:Lock:Gamesinline'..msg.chat_id_) then
Text =[[
*᥀︙قائمه الالعاب المتطوره اضغط للعب*
]]
keyboard = {} 
...
ChatLeave(dp.id_, SARW)
DevRio:srem(SARW.."Rio:Groups", dp.id_)
else
...
DevRio:incr(SARW..'Rio:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)
DevRio:set(SARW.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = DevRio:get(SARW.."Rio:Lock:Bots"..msg.chat_id_) 
for i=0,#mem_id do  
...
DevRio:sadd(SARW..'Rio:Tkeed:'..msg.chat_id_, msg.sender_user_id_)
GetInfo = https.request("https://api.telegram.org/bot"..TokenBot.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local JsonInfo = JSON.decode(GetInfo)
if JsonInfo.ok == true and #mem_id == i then
local Msgs = {}
...
if msg.content_.ID == "MessageChatDeleteMember" and tonumber(msg.content_.user_.id_) == tonumber(SARW) then 
DevRio:srem(SARW.."Rio:Groups", msg.chat_id_) 
...
if DevRio:get(SARW..'Rio:Lock:TagServr'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})    
end   
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then   
DevRio:incr(SARW..'Rio:EntryNumber'..msg.chat_id_..':'..os.date('%d'))  
elseif msg.content_.ID == "MessageChatDeleteMember" then   
DevRio:incr(SARW..'Rio:ExitNumber'..msg.chat_id_..':'..os.date('%d'))  
end
--     Source SARW     --
if text ==('تفعيل') and not SudoBot(msg) and ChCheck(msg) then
if not DevRio:get(SARW..'Rio:Lock:FreeBot'..SARW) then
if ChatType == 'pv' then
...
for i=0 , #admins do
if rio.members_[i].bot_info_ == false and rio.members_[i].status_.ID == "ChatMemberStatusEditor" then
DevRio:sadd(SARW..'Rio:Admins:'..msg.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevRio:srem(SARW..'Rio:Admins:'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
DevRio:sadd(SARW..'Rio:Admins:'..msg.chat_id_, admins[i].user_id_)
...
if rio.members_[i].status_.ID == "ChatMemberStatusCreator" then
DevRio:sadd(SARW.."Rio:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevRio:sadd(SARW.."Rio:RioConstructor:"..msg.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevRio:srem(SARW.."Rio:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevRio:srem(SARW.."Rio:RioConstructor:"..msg.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevRio:sismember(SARW..'Rio:Groups',msg.chat_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙المجموعه بالتاكيد مفعله', 1, 'md')
else
if tonumber(data.member_count_) < tonumber(DevRio:get(SARW..'Rio:Num:Add:Bot') or 0) and not SecondSudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙عدد اعضاء المجموعه اقل من ↫ *'..(DevRio:get(SARW..'Rio:Num:Add:Bot') or 0)..'* عضو', 1, 'md')
return false
end
ReplyStatus(msg,result.id_,"ReplyBy","᥀︙تم تفعيل المجموعه "..dp.title_)  
DevRio:sadd(SARW.."Rio:Groups",msg.chat_id_)
DevRio:sadd(SARW..'Rio:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
DevRio:set(SARW.."Rio:Groups:Links"..msg.chat_id_,LinkGroup) 
...
DevRio:set(SARW..'Save:UserName'..msg.sender_user_id_,data.username_)
end;end,nil) 
--     Source SARW     --
local ReFalse = tostring(msg.chat_id_)
if not DevRio:sismember(SARW.."Rio:Groups",msg.chat_id_) and not ReFalse:match("^(%d+)") and not SudoBot(msg) then
...
if DevRio:get(SARW..'Rio:Lock:Robot'..msg.chat_id_) then
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,dp) 
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..dp.id_)
DevRio:sadd(SARW..'Rio:Tkeed:'..msg.chat_id_, dp.id_)
...
if DevRio:get(SARW.."Rio:Lock:Join"..msg.chat_id_) then
...
if not Bot(msg) then 
local filter = DevRio:smembers(SARW.."Rio:FilterPhoto"..msg.chat_id_)
...
if not Bot(msg) then 
local filter = DevRio:smembers(SARW.."Rio:FilterAnimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if not Bot(msg) then 
local filter = DevRio:smembers(SARW.."Rio:FilterSteckr"..msg.chat_id_)
for k,v in pairs(filter) do
...
local DelGpRedRedods = DevRio:get(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
local GetGpTexts = DevRio:get(SARW..'Rio:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
if DelGpRedRedods == 'DelGpRedRedods' then
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙الرد ↫ '..msg.content_.text_..' للكلمه ↫ '..GetGpTexts..' تم حذفها',  1, "html")
DevRio:del(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevRio:srem(SARW..'Rio:Text:GpTexts'..GetGpTexts..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevRio:get(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRedod' then
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙الكلمه ( '..msg.content_.text_..' ) تم حذفها',  1, "html")
DevRio:del(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevRio:del(SARW..'Rio:Text:GpTexts'..msg.content_.text_..msg.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRedod'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevRio:get(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRed' then
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙الكلمه ( '..msg.content_.text_..' ) تم حذفها',  1, "html")
DevRio:del(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevRio:del(SARW..'Rio:Gif:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:Voice:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:Audio:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:Photo:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:Stecker:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:Video:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:File:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:del(SARW..'Rio:Text:GpRed'..msg.content_.text_..msg.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRed'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelAllRed = DevRio:get(SARW.."Rio:Add:AllRed"..msg.sender_user_id_)
if DelAllRed == 'DelAllRed' then
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙الكلمه ( '..msg.content_.text_..' ) تم حذفها',  1, "html")
DevRio:del(SARW.."Rio:Add:AllRed"..msg.sender_user_id_)
DevRio:del(SARW.."Rio:Gif:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Voice:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Audio:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Photo:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Stecker:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Video:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:File:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Text:AllRed"..msg.content_.text_)
DevRio:del(SARW.."Rio:Sudo:AllRed",msg.content_.text_)
return false
end
end
--     Source SARW     --
if text and text:match("^(.*)$") then
local SaveGpRedod = DevRio:get(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRedod == 'SaveGpRedod' then
local GetGpTexts = DevRio:get(SARW..'Rio:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevRio:smembers(SARW..'Rio:Text:GpTexts'..GetGpTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙᥀︙تم الغاء عملية حفظ الردود المتعدده للامر ↫ "..GetGpTexts ,  1, "md")
DevRio:del(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevRio:del(SARW..'Rio:Text:GpTexts'..GetGpTexts..msg.chat_id_)
DevRio:del(SARW..'Rio:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRedod'..msg.chat_id_,GetGpTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevRio:sadd(SARW..'Rio:Text:GpTexts'..GetGpTexts..msg.chat_id_,Text)
if #List == 4 then 
...
if text and not DevRio:get(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_) then
if DevRio:sismember(SARW..'Rio:Manager:GpRedod'..msg.chat_id_,text) then
local SARWTeam =  DevRio:smembers(SARW..'Rio:Text:GpTexts'..text..msg.chat_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, '['..SARWTeam[math.random(#SARWTeam)]..']' , 1, 'md')  
end
end
--     Source SARW     --
if text and text:match("^(.*)$") then
local SaveKt = DevRio:get(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
if SaveKt == 'SaveKt' then
local GetKtTexts = DevRio:get(SARW..'Rio:Add:KtTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevRio:smembers(SARW..'Rio:Text:KtTexts'..GetKtTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙᥀︙تم الغاء عملية حفظ اسئلة الكت  ↫ "..GetKtTexts ,  1, "md")
DevRio:del(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
DevRio:del(SARW..'Rio:Text:KtTexts'..GetKtTexts..msg.chat_id_)
DevRio:del(SARW..'Rio:Add:KtTexts'..msg.sender_user_id_..msg.chat_id_)
DevRio:srem(SARW..'Rio:Sudo:Kt'..msg.chat_id_,GetKtTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevRio:sadd(SARW..'Rio:Text:KtTexts'..GetKtTexts..msg.chat_id_,Text)
if #List == 100 then 
...
if text and not DevRio:get(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_) then
if DevRio:sismember(SARW..'Rio:Sudo:Kt'..msg.chat_id_,text) then
local DavidTeam =  DevRio:smembers(SARW..'Rio:Text:KtTexts'..text..msg.chat_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, '['..DavidTeam[math.random(#DavidTeam)]..']' , 1, 'md')  
end
end
--     Source SARW     --
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then 
local SaveGpRed = DevRio:get(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRed == 'SaveGpRed' then 
if text == 'الغاء' then
local DelManagerRep = DevRio:get(SARW..'DelManagerRep'..msg.chat_id_)
DevRio:srem(SARW..'Rio:Manager:GpRed'..msg.chat_id_,DelManagerRep)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ الرد', 1, 'md')
DevRio:del(SARW..'Rio:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
DevRio:del(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevRio:del(SARW..'DelManagerRep'..msg.chat_id_)
return false
end
DevRio:del(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
local SaveGpRed = DevRio:get(SARW..'Rio:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
if msg.content_.video_ then DevRio:set(SARW..'Rio:Video:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then DevRio:set(SARW..'Rio:File:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then DevRio:set(SARW..'Rio:Stecker:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_) 
end 
if msg.content_.voice_ then DevRio:set(SARW..'Rio:Voice:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_) 
end
if msg.content_.audio_ then DevRio:set(SARW..'Rio:Audio:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_) 
end
...
DevRio:set(SARW..'Rio:Photo:GpRed'..SaveGpRed..msg.chat_id_, photo_in_group) 
end
if msg.content_.animation_ then DevRio:set(SARW..'Rio:Gif:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_) 
end 
if msg.content_.text_ then
DevRio:set(SARW..'Rio:Text:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.text_)
end 
...
if msg.content_.text_ and not DevRio:get(SARW..'Rio:Lock:GpRed'..msg.chat_id_) then 
if DevRio:get(SARW..'Rio:Video:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevRio:get(SARW..'Rio:Video:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevRio:get(SARW..'Rio:File:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevRio:get(SARW..'Rio:File:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevRio:get(SARW..'Rio:Voice:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW..'Rio:Voice:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevRio:get(SARW..'Rio:Audio:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW..'Rio:Audio:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevRio:get(SARW..'Rio:Photo:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW..'Rio:Photo:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevRio:get(SARW..'Rio:Gif:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW..'Rio:Gif:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevRio:get(SARW..'Rio:Stecker:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevRio:get(SARW..'Rio:Stecker:GpRed'..msg.content_.text_..msg.chat_id_))
end
if DevRio:get(SARW..'Rio:Text:GpRed'..msg.content_.text_..msg.chat_id_) then
function SARWTeam(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevRio:get(SARW..'Rio:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevRio:get(SARW..'Rio:Text:GpRed'..msg.content_.text_..msg.chat_id_)
...
getUser(msg.sender_user_id_, SARWTeam)
end
end
--     Source SARW     --
text = msg.content_.text_
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then
local SaveAllRed = DevRio:get(SARW.."Rio:Add:AllRed"..msg.sender_user_id_)
if SaveAllRed == 'SaveAllRed' then
if text == 'الغاء' then
local DelSudoRep = DevRio:get(SARW..'DelSudoRep')
DevRio:del(SARW.."Rio:Sudo:AllRed",DelSudoRep)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ الرد', 1, 'md')
DevRio:del(SARW.."Rio:Add:AllText"..msg.sender_user_id_)
DevRio:del(SARW.."Rio:Add:AllRed"..msg.sender_user_id_)
DevRio:del(SARW.."DelSudoRep")
return false
end
DevRio:del(SARW.."Rio:Add:AllRed"..msg.sender_user_id_)
local SaveAllRed = DevRio:get(SARW.."Rio:Add:AllText"..msg.sender_user_id_)
if msg.content_.video_ then
DevRio:set(SARW.."Rio:Video:AllRed"..SaveAllRed, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then
DevRio:set(SARW.."Rio:File:AllRed"..SaveAllRed, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then
DevRio:set(SARW.."Rio:Stecker:AllRed"..SaveAllRed, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
DevRio:set(SARW.."Rio:Voice:AllRed"..SaveAllRed, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.audio_ then
DevRio:set(SARW.."Rio:Audio:AllRed"..SaveAllRed, msg.content_.audio_.audio_.persistent_id_)
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
DevRio:set(SARW.."Rio:Photo:AllRed"..SaveAllRed, photo_in_all_groups)
end
if msg.content_.animation_ then
DevRio:set(SARW.."Rio:Gif:AllRed"..SaveAllRed, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
DevRio:set(SARW.."Rio:Text:AllRed"..SaveAllRed, msg.content_.text_)
end 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ الرد الجديد', 1, 'md') 
DevRio:del(SARW.."Rio:Add:AllText"..msg.sender_user_id_)
DevRio:del(SARW..'DelSudoRep')
return false end end
if msg.content_.text_ and not DevRio:get(SARW..'Rio:Lock:AllRed'..msg.chat_id_) then
if DevRio:get(SARW.."Rio:Video:AllRed"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevRio:get(SARW.."Rio:Video:AllRed"..msg.content_.text_))
end
if DevRio:get(SARW.."Rio:File:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevRio:get(SARW.."Rio:File:AllRed"..msg.content_.text_))
end
if DevRio:get(SARW.."Rio:Voice:AllRed"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW.."Rio:Voice:AllRed"..msg.content_.text_))
end
if DevRio:get(SARW.."Rio:Audio:AllRed"..msg.content_.text_)  then
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW.."Rio:Audio:AllRed"..msg.content_.text_))
end
if DevRio:get(SARW.."Rio:Photo:AllRed"..msg.content_.text_)  then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW.."Rio:Photo:AllRed"..msg.content_.text_))
end
if  DevRio:get(SARW.."Rio:Gif:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevRio:get(SARW.."Rio:Gif:AllRed"..msg.content_.text_))
end
if DevRio:get(SARW.."Rio:Stecker:AllRed"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevRio:get(SARW.."Rio:Stecker:AllRed"..msg.content_.text_))
end
if DevRio:get(SARW.."Rio:Text:AllRed"..msg.content_.text_) then
function SARWTeam(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevRio:get(SARW..'Rio:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevRio:get(SARW.."Rio:Text:AllRed"..msg.content_.text_)
local Text = Text:gsub('#username',(username or 'لا يوجد')) 
local Text = Text:gsub('#name','['..result.first_name_..']')
local Text = Text:gsub('#id',msg.sender_user_id_)
local Text = Text:gsub('#edit',edit_msg)
...
if Type == "keed" and not DevRio:sismember(SARW..'Rio:Tkeed:'..msg.chat_id_, msg.sender_user_id_) then
...
if Type == "mute" and not DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, msg.sender_user_id_) then
DevRio:sadd(SARW..'Rio:Muted:'..msg.chat_id_,msg.sender_user_id_)
my_ide = msg.sender_user_id_
msgm = msg.id_
local num = 100
for i=1,tonumber(num) do
...
--     Source SARW     --
--       Spam Check       --
if not VipMem(msg) and msg.content_.ID ~= "MessageChatAddMembers" and DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= SARW then
floods = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(DevRio:get(SARW.."Rio:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
DevRio:setex(SARW.."Rio:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
--     Source SARW     --
...
if Constructor(msg) or tonumber(msg.sender_user_id_) == tonumber(SARW) then
DevRio:set(SARW..'Rio:PinnedMsg'..msg.chat_id_,msg.content_.message_id_)
else
local pin_id = DevRio:get(SARW..'Rio:PinnedMsg'..msg.chat_id_)
if pin_id and DevRio:get(SARW..'Rio:Lock:Pin'..msg.chat_id_) then
pinmsg(msg.chat_id_,pin_id,0)
end
end
end
if DevRio:get(SARW..'Rio:viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
DevRio:del(SARW..'Rio:viewget'..msg.sender_user_id_)
else
...
if DevRio:get(SARW..'Rio:Lock:Forwards'..msg.chat_id_) then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
if DevRio:get(SARW..'Rio:Lock:Photo'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
if DevRio:get(SARW..'Rio:Lock:Links'..msg.chat_id_) then
if msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if DevRio:get(SARW..'Rio:Lock:Tags'..msg.chat_id_) then
if msg.content_.caption_:match("@") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("#") then
if DevRio:get(SARW..'Rio:Lock:Hashtak'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://") or msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Xx][Yy][Zz]") or msg.content_.caption_:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
if DevRio:get(SARW..'Rio:Lock:WebLinks'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[\216-\219][\128-\191]") then
if DevRio:get(SARW..'Rio:Lock:Arabic'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]") then
if DevRio:get(SARW..'Rio:Lock:English'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if DevRio:get(SARW..'Rio:Lock:Markdown'..msg.chat_id_) then
if not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevRio:get(SARW..'Rio:Lock:Document'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if not VipMem(msg) then
if DevRio:get(SARW..'Rio:Lock:Inline'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source SARW     --
...
elseif msg.content_.ID == "MessageSticker" then
if not VipMem(msg) then
if DevRio:get(SARW..'Rio:Lock:Stickers'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevRio:get(SARW..'Rio:Groups:Welcomes'..msg.chat_id_) then
Welcomes = DevRio:get(SARW..'Rio:Groups:Welcomes'..msg.chat_id_)
else
Welcomes = '• نورت حبي \n• firstname \n• @username'
end
...
if DevRio:get(SARW.."Rio:Lock:Welcome"..msg.chat_id_) then
getUser(msg.sender_user_id_,get_welcome)
end
--     Source SARW     --
--      New User Add      --
elseif msg.content_.ID == "MessageChatAddMembers" then
if not DevRio:get(SARW..'Rio:Lock:BotWelcome') then 
tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = SARW,offset_ = 0,limit_ = 1},function(extra,rio,success) 
for i=0,#msg.content_.members_ do    
BotWelcome = msg.content_.members_[i].id_    
if BotWelcome and BotWelcome == tonumber(SARW) then 
if DevRio:sismember(SARW..'Rio:Groups',msg.chat_id_) then BotText = "مفعله في السابق\n᥀︙ارسل ↫ الاوامر واستمتع بالمميزيات" else BotText = "معطله يجب رفعي مشرف\n᥀︙بعد ذلك يرجى ارسال امر ↫ تفعيل\n᥀︙سيتم رفع الادمنيه والمنشئ تلقائيا" end 
if DevRio:get(SARW.."Rio:Text:BotWelcome") then RioText = DevRio:get(SARW.."Rio:Text:BotWelcome") else RioText = "᥀︙مرحبا انا بوت اسمي "..NameBot.."\n᥀︙حالة المجموعه ↫ "..BotText.."\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉" end 
if DevRio:get(SARW.."Rio:Photo:BotWelcome") then RioPhoto = DevRio:get(SARW.."Rio:Photo:BotWelcome") elseif rio.photos_[0] then RioPhoto = rio.photos_[0].sizes_[1].photo_.persistent_id_ else RioPhoto = nil end 
...
if DevRio:get(SARW..'Rio:Lock:Contact'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source SARW     --
...
if DevRio:get(SARW..'Rio:Lock:Music'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevRio:get(SARW..'Rio:Lock:Voice'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevRio:get(SARW..'Rio:Lock:Location'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
return
end
if msg.content_.caption_ then
...
if DevRio:get(SARW..'Rio:Lock:Videos'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevRio:get(SARW..'Rio:Lock:Gifs'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevRio:get(SARW..'Rio:Lock:Text'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
...
if not DevRio:get(SARW..'Rio:Spam:Text'..msg.chat_id_) then
sens = 400
else
sens = tonumber(DevRio:get(SARW..'Rio:Spam:Text'..msg.chat_id_))
end
if DevRio:get(SARW..'Rio:Lock:Spam'..msg.chat_id_) and string.len(msg.content_.text_) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
--     Source SARW     --
if DevRio:get(SARW.."Rio:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_,"᥀︙تم الغاء حفظ الرابط")       
DevRio:del(SARW.."Rio:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) 
return false
end
if msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)") then
local Link = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
DevRio:set(SARW.."Rio:Groups:Links"..msg.chat_id_,Link)
...
if DevRio:get('SARWTeam:'..SARW.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevRio:del('SARWTeam:'..SARW..'id:user'..msg.chat_id_)  
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
DevRio:del('SARWTeam:'..SARW.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevRio:del('SARWTeam:'..SARW.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevRio:get('SARWTeam:'..SARW..'id:user'..msg.chat_id_)  
DevRio:incrby(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..iduserr,numadded)
Dev_Rio(msg.chat_id_, msg.id_,  1, "᥀︙تم اضافة "..numadded..' رساله', 1, 'md')
DevRio:del('SARWTeam:'..SARW..'id:user'..msg.chat_id_) 
end
end
if text and Constructor(msg) then 
if DevRio:get('SARWTeam:'..SARW.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevRio:del('SARWTeam:'..SARW..'ids:user'..msg.chat_id_)  
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
DevRio:del('SARWTeam:'..SARW.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevRio:del('SARWTeam:'..SARW.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevRio:get('SARWTeam:'..SARW..'ids:user'..msg.chat_id_)  
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..iduserr,numadded)  
...
if not DevRio:get(SARW.."Rio:Lock:Fshar"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if DevRio:get(SARW.."Rio:Lock:Farsi"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ReplyStatus(msg,msg.sender_user_id_,"WrongWay","᥀︙ممنوع التكلم بالغه الفارسيه هنا")  
end end
if text and (text:match("ڬ") or text:match("ٺ") or text:match("چ") or text:match("ڇ") or text:match("ڿ") or text:match("ڀ") or text:match("ڎ") or text:match("ݫ") or text:match("ژ") or text:match("ڟ") or text:match("ݜ") or text:match("ڸ") or text:match("پ") or text:match("۴") or text:match("مک") or text:match("زدن") or text:match("دخترا") or text:match("دیوث") or text:match("کلیپشن") or text:match("خوششون") or text:match("میدا") or text:match("که") or text:match("بدانیم") or text:match("باید") or text:match("زناشویی") or text:match("آموزش") or text:match("راحتی") or text:match("خسته") or text:match("بیام") or text:match("بپوشم") or text:match("كرمه")) then
if DevRio:get(SARW.."Rio:Lock:FarsiBan"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ChatKick(msg.chat_id_, msg.sender_user_id_)
end end 
if text and (text:match("خره بالله") or text:match("خبربك") or text:match("كسدينربك") or text:match("خرب بالله") or text:match("خرب الله") or text:match("خره بربك") or text:match("الله الكواد") or text:match("خره بمحمد") or text:match("كسم الله") or text:match("كسم ربك") or text:match("كسربك") or text:match("كسختالله") or text:match("كسخت الله") or text:match("خره بدينك") or text:match("خرهبدينك") or text:match("كسالله") or text:match("خربالله")) then
if not DevRio:get(SARW.."Rio:Lock:Kfr"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if not DevRio:get(SARW.."Rio:Lock:Taf"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
local List = DevRio:smembers(SARW..'Rio:Groups') 
local BotName = (DevRio:get(SARW.."Rio:NameBot") or 'سرو')
local GetJson = '{"BotId": '..SARW..',"BotName": "'..BotName..'","GroupsList":{'  
for k,v in pairs(List) do 
LinkGroups = DevRio:get(SARW.."Rio:Groups:Links"..v)
Welcomes = DevRio:get(SARW..'Rio:Groups:Welcomes'..v) or ''
Welcomes = Welcomes:gsub('"',"") Welcomes = Welcomes:gsub("'","") Welcomes = Welcomes:gsub(",","") Welcomes = Welcomes:gsub("*","") Welcomes = Welcomes:gsub(";","") Welcomes = Welcomes:gsub("`","") Welcomes = Welcomes:gsub("{","") Welcomes = Welcomes:gsub("}","") 
RioConstructors = DevRio:smembers(SARW..'Rio:RioConstructor:'..v)
Constructors = DevRio:smembers(SARW..'Rio:BasicConstructor:'..v)
BasicConstructors = DevRio:smembers(SARW..'Rio:Constructor:'..v)
Managers = DevRio:smembers(SARW..'Rio:Managers:'..v)
Admis = DevRio:smembers(SARW..'Rio:Admins:'..v)
Vips = DevRio:smembers(SARW..'Rio:VipMem:'..v)
if k == 1 then
GetJson = GetJson..'"'..v..'":{'
else
GetJson = GetJson..',"'..v..'":{'
...
GetJson = GetJson..'}}'
local File = io.open('./'..SARW..'.json', "w")
File:write(GetJson)
File:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..SARW..'.json', '᥀︙يحتوي الملف على ↫ '..#List..' مجموعه',dl_cb, nil)
io.popen('rm -rf ./'..SARW..'.json')
end
...
--     Source SARW     --
if DevRio:get(SARW.."SET:GAME"..msg.chat_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
Dev_Rio( msg.chat_id_, msg.id_, 1,"᥀︙يوجد فقط ( 6 ) اختيارات\n᥀︙ارسل اختيارك مره اخرى", 1, "md")    
return false  end 
local GETNUM = DevRio:get(SARW.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevRio:del(SARW.."SET:GAME"..msg.chat_id_)   
Dev_Rio( msg.chat_id_, msg.id_, 1,'᥀︙*المحيبس باليد رقم* ↫ '..NUM..'\n᥀︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, "md") 
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
...
if DevRio:get(SARW..'DevRio4'..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_, "᥀︙تم الغاء الامر")
DevRio:del(SARW..'DevRio4'..msg.sender_user_id_)
return false  end 
DevRio:del(SARW..'DevRio4'..msg.sender_user_id_)
...
DevRio:set(SARW..'Rio:ChId',data.id_)
else
...
--     Source SARW     --
if DevRio:get(SARW.."Rio:DevText"..msg.chat_id_..":" .. msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
DevRio:del(SARW.."Rio:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
return false 
end 
DevRio:del(SARW.."Rio:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
local DevText = msg.content_.text_:match("(.*)")
DevRio:set(SARW.."DevText", DevText)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ كليشة المطور", 1, "md")
end
if DevRio:get(SARW..'Rio:NameBot'..msg.sender_user_id_) == 'msg' then
if text and text:match("^الغاء$") then 
DevRio:del(SARW..'Rio:NameBot'..msg.sender_user_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
return false 
end 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ اسم البوت ', 1, 'html')
DevRio:del(SARW..'Rio:NameBot'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:NameBot', text)
return false 
end
--     Source SARW     --
if msg.chat_id_ then
...
NameBot = (DevRio:get(SARW..'Rio:NameBot') or 'سرو')
local SARWTeam = {' كول حبيبي ؟ اني '..NameBot..' ',' وياك القميل '..NameBot..' ',' اسمي القميل '..NameBot..' '}
DevRio2 = math.random(#SARWTeam) 
Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam[DevRio2] , 1, 'html') 
return false
end
if text == 'اسم البوت' or text == 'البوت شنو اسمه' or text == 'شسمه البوت' or text == 'البوت شسمه' then
NameBot = (DevRio:get(SARW..'Rio:NameBot') or 'سرو') 
local SARWTeam = {"اسمي القميل "..NameBot.." "} 
DevRio2 = math.random(#SARWTeam) 
Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam[DevRio2] , 1, 'html') 
return false
end
if text and text == (DevRio:get(SARW..'Rio:NameBot') or 'سرو') then 
NameBot = (DevRio:get(SARW..'Rio:NameBot') or 'سرو')
...
if tonumber((DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙لم تربح اي نقطه\n᥀︙ارسل ↫ الالعاب للعب', 1, 'md')
else 
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙عدد النقاط التي ربحتها ↫ '..(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_)), 1, 'md')
end
end
if text ==  'حذف نقاطي' and ChCheck(msg) or text ==  'مسح نقاطي' and ChCheck(msg) then DevRio:del(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_) Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم حذف جميع نقاطك', 1, 'md') end
--     Source SARW     --
if text == 'سمايلات' and ChCheck(msg) or text == 'السمايلات' and ChCheck(msg) or text == '↫ سمايلات ᥀' and ChCheck(msg) then
if not DevRio:get(SARW..'Rio:Lock:Games'..msg.chat_id_) then
DevRio2 = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🏆','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🛎','📤','🎗','🏵','🎖','🏆','🥌','??','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔️','🛡','🔮','🌡','💣','⏱','🛢','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
name = string.gsub(name,'🎚','🎚')
name = string.gsub(name,'☎️','☎️')
SARWTeam = '᥀︙اول واحد يدز هذا السمايل يربح ↫ '..name
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum'..msg.chat_id_) and not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
SARWTeam = '᥀︙مبروك لقد ربحت في اللعبه \n᥀︙ارسل ↫ سمايلات للعب مره اخرى'
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
end
DevRio:set(SARW..'Rio:Games:Ids'..msg.chat_id_,true)
end
...
SARWTeam = '᥀︙اول واحد يرتبها يربح ↫ '..name
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum'..msg.chat_id_) and not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then
...
if not DevRio:get(SARW..'Rio:Lock:Games'..msg.chat_id_) then
Num = math.random(1,6)
DevRio:set(SARW.."GAMES"..msg.chat_id_,Num) 
TEST = [[
➀     ➁     ➂     ➃     ➄     ➅
↓     ↓     ↓     ↓     ↓     ↓
👊 ‹› 👊🏻 ‹› 👊🏼 ‹› 👊🏽 ‹› 👊🏾 ‹› 👊🏿
...
DevRio:setex(SARW.."SET:GAME"..msg.chat_id_, 100, true)  
return false  
end end
...
SARWTeam = '᥀︙اول واحد يحلها يربح ↫ '..name
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum'..msg.chat_id_) and not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then
...
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum2'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
SARWTeam = '᥀︙ما معنى هذا السمايل :؟ ↫ '..name
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum2'..msg.chat_id_) and not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
...
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum3'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
SARWTeam = '᥀︙ما هو عكس كلمة ↫ '..name
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum3'..msg.chat_id_) and not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
...
DevRio2 = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','📥','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧚‍♂️','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕓','🕤','⌛️','📅','👩‍⚖️','👨‍🎨'};
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum4'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
name = string.gsub(name,'😸','😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠️','💀💀💀☠️💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼')
...
name = string.gsub(name,'👩‍⚖️','👨‍⚖️👨‍⚖️👨‍⚖️👨‍⚖️👨‍⚖️👩‍⚖️👨‍⚖️👨‍⚖️')
name = string.gsub(name,'👨‍🎨','👩‍🎨👩‍🎨👨‍🎨👩‍🎨👩‍🎨👩‍🎨👩‍🎨👩‍🎨')
SARWTeam = '᥀︙اول واحد يطلع المختلف يربح\n{'..name..'} '
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum4'..msg.chat_id_) and not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
...
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum5'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
SARWTeam = '᥀︙اكمل المثال التالي ↫ ['..name..']'
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum5'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevRio:del(SARW..'Rio:GameNum5'..msg.chat_id_)
...
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum6'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
SARWTeam = '᥀︙اكمل المعادله التاليه ↫ ⤈\n{'..name..'} '
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum6'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevRio:del(SARW..'Rio:GameNum6'..msg.chat_id_)
...
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum7'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
SARWTeam = '᥀︙ما معنى كلمة ↫ '..name
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md')
return false
end end
if text == DevRio:get(SARW..'Rio:GameNum7'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevRio:del(SARW..'Rio:GameNum7'..msg.chat_id_)
...
name = DevRio2[math.random(#DevRio2)]
DevRio:set(SARW..'Rio:GameNum8'..msg.chat_id_,name)
DevRio:del(SARW..'Rio:Games:Ids'..msg.chat_id_)
...
if text == DevRio:get(SARW..'Rio:GameNum8'..msg.chat_id_) then
if not DevRio:get(SARW..'Rio:Games:Ids'..msg.chat_id_) then 
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevRio:del(SARW..'Rio:GameNum8'..msg.chat_id_)
...
--     Source SARW     --
if DevRio:get(SARW.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
Dev_Rio(msg.chat_id_, msg.id_, 1,"᥀︙عذرا لا يمكنك تخمين عدد اكبر من الـ20 خمن رقم ما بين الـ1 والـ20", 1, 'md')
return false  end 
local GETNUM = DevRio:get(SARW.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevRio:del(SARW..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevRio:del(SARW.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙*التخمين الصحيح هو* ↫ '..NUM..'\n᥀︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
DevRio:incrby(SARW..'Set:Num'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(DevRio:get(SARW..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
DevRio:del(SARW..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevRio:del(SARW.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙*التخمين الصحيح هو* ↫ '..GETNUM..'\n᥀︙*للاسف لقد خسرت حاول مره اخرى لتخمين الرقم الصحيح*', 1, 'md')
else
if tonumber(DevRio:get(SARW..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 1 then
SetNum = 'محاولتان فقط'
elseif tonumber(DevRio:get(SARW..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 2 then
...
DevRio:set(SARW.."GAMES:NUM"..msg.chat_id_,Num) 
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙اهلا بك عزيزي في لعبة التخمين ↫ ⤈\n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n᥀︙سيتم تخمين عدد ما بين الـ1 والـ20 اذا تعتقد انك تستطيع الفوز جرب واللعب الان .\n᥀︙ملاحظه لديك ثلاث محاولات فقط فكر قبل ارسال تخمينك !', 1, 'md')
DevRio:setex(SARW.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_, 100, true)  
return false  
end
end
--     Source SARW     --
...
if not DevRio:get(SARW..'Rio:Lock:Games'..msg.chat_id_) then
DevRio:del(SARW.."Rio:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevRio:del(SARW..'Rio:ListRolet'..msg.chat_id_)  
DevRio:setex(SARW.."Rio:StartRolet"..msg.chat_id_..msg.sender_user_id_,3600,true)  
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙حسنا لنلعب , ارسل عدد اللاعبين للروليت .', 1, 'md')
return false  
end
end
if text and text:match("^(%d+)$") and DevRio:get(SARW.."Rio:StartRolet"..msg.chat_id_..msg.sender_user_id_) then
if text == "1" then
Text = "᥀︙لا استطيع بدء اللعبه بلاعب واحد فقط"
else
DevRio:set(SARW.."Rio:NumRolet"..msg.chat_id_..msg.sender_user_id_,text)  
Text = '᥀︙تم بدء تسجيل اللسته يرجى ارسال المعرفات \n᥀︙الفائز يحصل على 5 نقاط عدد المطلوبين ↫ '..text..' لاعب'
end
DevRio:del(SARW.."Rio:StartRolet"..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,Text)
return false
end
if text and text:match('^(@[%a%d_]+)$') and DevRio:get(SARW.."Rio:NumRolet"..msg.chat_id_..msg.sender_user_id_) then 
if DevRio:sismember(SARW..'Rio:ListRolet'..msg.chat_id_,text) then
...
DevRio:sadd(SARW..'Rio:ListRolet'..msg.chat_id_,text)
local CountAdd = DevRio:get(SARW.."Rio:NumRolet"..msg.chat_id_..msg.sender_user_id_)
local CountAll = DevRio:scard(SARW..'Rio:ListRolet'..msg.chat_id_)
local CountUser = CountAdd - CountAll
if tonumber(CountAll) == tonumber(CountAdd) then 
DevRio:del(SARW.."Rio:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevRio:setex(SARW.."Rio:WittingStartRolet"..msg.chat_id_..msg.sender_user_id_,1400,true) 
...
local Tsl = DevRio:get(SARW..'Rio:Tsl')
local Text = [[
...
--     Source SARW     --
if text == "الرابط" then
local Link = DevRio:get(SARW..'Rio:Link')
local Text = [[
...
if not DevRio:get(SARW..'Rio:Lock:Games'..msg.chat_id_) then
local Games = DevRio:get(SARW..'Rio:Games')
local Text = [[
...
DevRio0 = (DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_) * 50)
DevRio:incrby(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_,DevRio0)
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙تم بيع '..(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_))..' من نقاطك\n᥀︙كل نقطه تساوي 50 رساله', 'md')
DevRio:del(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_)
end
end
--     Source SARW     --
...
if rio.members_[i].status_.ID == "ChatMemberStatusCreator" then  
Manager_id = admins[i].user_id_  
DevRio:sadd(SARW..'Rio:BasicConstructor:'..msg.chat_id_,Manager_id)  
DevRio:sadd(SARW..'Rio:RioConstructor:'..msg.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
local msguser = tonumber(DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_))
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local cont = (tonumber(DevRio:get(SARW..'Rio:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
local user_nkt = tonumber(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
if result.username_ then username = '@'..result.username_ else username = 'لا يوجد' end
...
DevRio:setex(SARW..'DevRio4'..msg.sender_user_id_,360,true)
...
function SARWTeam(extra,result,success)
if DevRio:get(SARW.."Rio:Send:Bot"..SARW) and not RioSudo(msg) then 
send(msg.chat_id_, msg.id_,"᥀︙الاذاعه معطله من قبل المطور الاساسي")
return false
end
local GpList = DevRio:smembers(SARW.."Rio:Groups")
for k,v in pairs(GpList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
local PvList = DevRio:smembers(SARW.."Rio:Users")
for k,v in pairs(PvList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم اذاعة رسالتك بالتوجيه \n᥀︙‏في ↫ ❨ '..#GpList..' ❩ مجموعه \n᥀︙والى ↫ ❨ '..#PvList..' ❩ مشترك \n ✓', 1, 'md')
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),SARWTeam)
end
end
--     Source SARW     --
if text == "مشاهده المنشور" and ChCheck(msg) or text == "مشاهدات المنشور" and ChCheck(msg) or text == "عدد المشاهدات" and ChCheck(msg) then
DevRio:set(SARW..'Rio:viewget'..msg.sender_user_id_,true)
...
Text = [[
Welcome To Source
᥀︙[TeAm SARW](https://t.me/YY55c)
]]
keyboard = {} 
keyboard.inline_keyboard = {{{text = '᥀ Source Channel',url="https://t.me/YY55c"}},{{text = '᥀ Developer',url="t.me/lIHHHl"},{text = '᥀ Tws SARW',url="https://t.me/PLJBOT"}},{{text= '᥀ Exp Source .',url="https://t.me/PHPHB"}}}
local msg_id = msg.id_/2097152/0.5
...
if DevRio:get(SARW.."Rio:Kick:Me"..msg.chat_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevRio:set(SARW..'yes'..msg.sender_user_id_, 'delyes')
DevRio:set(SARW..'no'..msg.sender_user_id_, 'delno')
...
DevRio:set(SARW.."Rio:Kick:Me"..msg.chat_id_, true)
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل امر اطردني'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل اطردني' and Manager(msg) and ChCheck(msg) then
DevRio:del(SARW.."Rio:Kick:Me"..msg.chat_id_)
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل امر اطردني'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
end
--     Source SARW     --
if text == "نزلني" and ChCheck(msg) then
if DevRio:get(SARW.."Rio:Del:Me"..msg.chat_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevRio:set(SARW..'yesdel'..msg.sender_user_id_, 'delyes')
DevRio:set(SARW..'nodel'..msg.sender_user_id_, 'delno')
...
DevRio:set(SARW.."Rio:Del:Me"..msg.chat_id_, true)
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل امر نزلني'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل نزلني' and BasicConstructor(msg) and ChCheck(msg) then
DevRio:del(SARW.."Rio:Del:Me"..msg.chat_id_)
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل امر نزلني'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
end
--     Source SARW     --
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:TagAll'..msg.chat_id_)
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:TagAll'..msg.chat_id_,true)
end
if Admin(msg) then
if text == "تاك للكل" and ChCheck(msg) then
if not DevRio:get(SARW..'Rio:Lock:TagAll'..msg.chat_id_) then
function TagAll(dp1,dp2)
local text = "᥀︙وينكم يالربع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
i = 0
for k, v in pairs(dp2.members_) do
i = i + 1
if DevRio:get(SARW..'Save:UserName'..v.user_id_) then
text = text..i.."~ : [@"..DevRio:get(SARW..'Save:UserName'..v.user_id_).."]\n"
else
...
local EntryNumber = (DevRio:get(SARW..'Rio:EntryNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local ExitNumber = (DevRio:get(SARW..'Rio:ExitNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local MsgNumberDay = (DevRio:get(SARW..'Rio:MsgNumberDay'..msg.chat_id_..':'..os.date('%d')) or 0)
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل تفاعلي'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:msg:Rio'..msg.chat_id_) 
end
if text == "تفعيل تفاعلي" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل تفاعلي'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:msg:Rio'..msg.chat_id_,true)  
end
if DevRio:get(SARW.."Rio:msg:Rio"..msg.chat_id_) then
if msg.content_.ID then
get_msg = DevRio:get(SARW.."Rio:msg:Rio"..msg.sender_user_id_..":"..msg.chat_id_) or 0
gms = get_msg + 1
DevRio:setex(SARW..'Rio:msg:Rio'..msg.sender_user_id_..":"..msg.chat_id_,86400,gms)
end
...
function prom_reply(extra, result, success) 
get_msg = DevRio:get(SARW.."Rio:msg:Rio"..result.sender_user_id_..":"..msg.chat_id_) or 0
...
--     Source SARW     --
if text == "جهاتي" and ChCheck(msg) or text == "اضافاتي" and ChCheck(msg) then add = (tonumber(DevRio:get(SARW..'Rio:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0) Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙عدد جهاتك المضافه ↫ *❨ "..add.." ❩* ", 1, 'md') end
if text == "تعديلاتي" or text == "سحكاتي" and ChCheck(msg) then local edit_msg = DevRio:get(SARW..'Rio:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0  Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙عدد تعديلاتك ↫ *❨ "..edit_msg.." ❩* ", 1, 'md') end
...
if text == 'مسح جهاتي' or text == 'مسح اضافاتي' or text == 'حذف جهاتي' or text == 'حذف اضافاتي' then DevRio:del(SARW..'Rio:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم حذف جميع جهاتك المضافه' , 1, 'md') end
--     Source SARW     --
if text and text:match('^هينه @(.*)') and ChCheck(msg) or text and text:match('^هينها @(.*)') and ChCheck(msg) then 
if not DevRio:get(SARW..'Rio:Lock:Stupid'..msg.chat_id_) then
local username = text:match('^هينه @(.*)') or text:match('^هينها @(.*)') 
function SARWTeam(extra,result,success)
if result.id_ then  
if tonumber(result.id_) == tonumber(SARW) then  
...
if DevRio:sismember(SARW.."Rio:RioConstructor:"..msg.chat_id_,result.id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local SARWTeam = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md') 
local SARWTeam = { "لكك جرجف @"..username.." احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش لكك فاشل @"..username.." لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","حبيبي @"..username.." راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪","دمشي لك @"..username.." ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع @"..username.." متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪",}
Dev_Rio(msg.chat_id_, result.id_, 1,''..SARWTeam[math.random(#SARWTeam)], 1, 'html') 
else  
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو غير موجود في المجموعه', 1, 'md') 
end 
end 
resolve_username(username,SARWTeam)
end
end
--     Source SARW     --
...
if tonumber(result.sender_user_id_) == tonumber(SARW) then 
Dev_Rio(msg.chat_id_, msg.id_, 1, 'شو تمضرط اكو واحد يهين نفسه؟🤔👌🏿', 1, 'md') 
return false  
end  
...
if DevRio:sismember(SARW.."Rio:RioConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local SARWTeam = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md') 
local SARWTeam = {"لكك جرجف احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش فاشل لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","دمشي لك ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪","حبيبي راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪"} 
Dev_Rio(msg.chat_id_, result.id_, 1,''..SARWTeam[math.random(#SARWTeam)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local SARWTeam = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md') 
local SARWTeam = {"مواححح افيش عافيههه😍🔥💗","امممووااهحح شهلعسل🥺🍯💘","مواححح،ءوفف اذوب🤤💗"} 
Dev_Rio(msg.chat_id_, result.id_, 1,''..SARWTeam[math.random(#SARWTeam)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local SARWTeam = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md') 
local SARWTeam = {"تتعال ححب محتاجيك🙂🍭","تعال يولل استاذكك ايريدككك😒🔪","يمعوود تعاال يريدوكك🤕♥️","تعال لكك ديصيحوك😐🖤"} 
Dev_Rio(msg.chat_id_, result.id_, 1,''..SARWTeam[math.random(#SARWTeam)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local SARWTeam = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Rio(msg.chat_id_, msg.id_, 1,SARWTeam, 1, 'md') 
local SARWTeam = { "تتعال ححب @"..username.." محتاجيك🙂🍭","تعال يولل @"..username.." استاذكك ايريدككك😒🔪","يمعوود @"..username.." تعاال يريدوكك🤕♥️","تعال لكك @"..username.." ديصيحوك😐🖤",}
Dev_Rio(msg.chat_id_, result.id_, 1,''..SARWTeam[math.random(#SARWTeam)], 1, 'html') 
else  
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙العضو غير موجود في المجموعه', 1, 'md') 
end 
...
if DevRio:sismember(SARW..'Rio:RioSudo:',result.sender_user_id_) then
riosudo = 'المطورين الاساسيين • ' else riosudo = '' end
if DevRio:sismember(SARW..'Rio:SecondSudo:',result.sender_user_id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevRio:sismember(SARW..'Rio:SudoBot:',result.sender_user_id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevRio:sismember(SARW..'Rio:Owner:'..msg.chat_id_, result.sender_user_id_) then
owner = 'المالكين • ' else owner = '' end
if DevRio:sismember(SARW..'Rio:BasicConstructor:'..msg.chat_id_, result.sender_user_id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevRio:sismember(SARW..'Rio:Constructor:'..msg.chat_id_, result.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevRio:sismember(SARW..'Rio:Managers:'..msg.chat_id_, result.sender_user_id_) then
manager = 'المدراء • ' else manager = '' end
if DevRio:sismember(SARW..'Rio:Admins:'..msg.chat_id_, result.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevRio:sismember(SARW..'Rio:VipMem:'..msg.chat_id_, result.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevRio:sismember(SARW..'Rio:Cleaner:'..msg.chat_id_, result.sender_user_id_) then
...
DevRio:srem(SARW..'Rio:RioSudo:', result.sender_user_id_)
DevRio:srem(SARW..'Rio:SecondSudo:', result.sender_user_id_)
DevRio:srem(SARW..'Rio:SudoBot:', result.sender_user_id_)
DevRio:srem(SARW..'Rio:Owner:'..msg.chat_id_,result.sender_user_id_)
DevRio:srem(SARW..'Rio:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
DevRio:srem(SARW..'Rio:Constructor:'..msg.chat_id_,result.sender_user_id_)
DevRio:srem(SARW..'Rio:Managers:'..msg.chat_id_, result.sender_user_id_)
DevRio:srem(SARW..'Rio:Admins:'..msg.chat_id_, result.sender_user_id_)
DevRio:srem(SARW..'Rio:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevRio:srem(SARW..'Rio:Cleaner:'..msg.chat_id_, result.sender_user_id_)
...
if DevRio:sismember(SARW..'Rio:RioSudo:',result.id_) then
Riosudo = 'المطورين الاساسيين • ' else Riosudo = '' end
if DevRio:sismember(SARW..'Rio:SecondSudo:',result.id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevRio:sismember(SARW..'Rio:SudoBot:',result.id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevRio:sismember(SARW..'Rio:Owner:'..msg.chat_id_, result.id_) then
owner = 'المالكين • ' else owner = '' end
if DevRio:sismember(SARW..'Rio:BasicConstructor:'..msg.chat_id_, result.id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevRio:sismember(SARW..'Rio:Constructor:'..msg.chat_id_, result.id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevRio:sismember(SARW..'Rio:Managers:'..msg.chat_id_, result.id_) then
manager = 'المدراء • ' else manager = '' end
if DevRio:sismember(SARW..'Rio:Admins:'..msg.chat_id_, result.id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevRio:sismember(SARW..'Rio:VipMem:'..msg.chat_id_, result.id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevRio:sismember(SARW..'Rio:Cleaner:'..msg.chat_id_, result.id_) then
cleaner = 'المنظفين • ' else cleaner = ''
end
...
if RioDelAll(msg.sender_user_id_,msg.chat_id_) == 'sudoid' then
DevRio:srem(SARW..'Rio:RioSudo:', result.id_)
DevRio:srem(SARW..'Rio:SecondSudo:', result.id_)
DevRio:srem(SARW..'Rio:SudoBot:', result.id_)
DevRio:srem(SARW..'Rio:Owner:'..msg.chat_id_,result.id_)
DevRio:srem(SARW..'Rio:BasicConstructor:'..msg.chat_id_,result.id_)
DevRio:srem(SARW..'Rio:Constructor:'..msg.chat_id_,result.id_)
DevRio:srem(SARW..'Rio:Managers:'..msg.chat_id_, result.id_)
DevRio:srem(SARW..'Rio:Admins:'..msg.chat_id_, result.id_)
DevRio:srem(SARW..'Rio:VipMem:'..msg.chat_id_, result.id_)
DevRio:srem(SARW..'Rio:Cleaner:'..msg.chat_id_, result.id_)
elseif RioDelAll(msg.sender_user_id_,msg.chat_id_) == 'riosudo' then
DevRio:srem(SARW..'Rio:SecondSudo:', result.id_)
DevRio:srem(SARW..'Rio:SudoBot:', result.id_)
DevRio:srem(SARW..'Rio:Owner:'..msg.chat_id_,result.id_)
...
DevRio:sadd(SARW..'Rio:RioSudo:',result.sender_user_id_)
...
if result.id_ then
DevRio:sadd(SARW..'Rio:RioSudo:',result.id_)
...
DevRio:sadd(SARW..'Rio:RioSudo:',user)
...
DevRio:srem(SARW..'Rio:RioSudo:',user)
...
function sudo_reply(extra, result, success)
DevRio:sadd(SARW..'Rio:SecondSudo:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevRio:sadd(SARW..'Rio:SecondSudo:',result.id_)
...
DevRio:sadd(SARW..'Rio:SecondSudo:',user)
...
DevRio:srem(SARW..'Rio:SecondSudo:',user)
...
function sudo_reply(extra, result, success)
DevRio:sadd(SARW..'Rio:SudoBot:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevRio:sadd(SARW..'Rio:SudoBot:',result.id_)
...
DevRio:sadd(SARW..'Rio:SudoBot:',user)
...
DevRio:srem(SARW..'Rio:SudoBot:',user)
...
DevRio:sadd(SARW..'Rio:Owner:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevRio:sadd(SARW..'Rio:Owner:'..msg.chat_id_,result.id_)
...
DevRio:sadd(SARW..'Rio:Owner:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:Owner:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
if not RioConstructor(msg) and DevRio:get(SARW.."Rio:Lock:ProSet"..msg.chat_id_) then 
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙لاتستطيع رفع احد وذالك بسبب تعطيل الرفع', 1, 'md')
return false
end
DevRio:sadd(SARW..'Rio:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevRio:sadd(SARW..'Rio:BasicConstructor:'..msg.chat_id_,result.id_)
...
DevRio:sadd(SARW..'Rio:BasicConstructor:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:BasicConstructor:'..msg.chat_id_,user)
...
DevRio:sadd(SARW..'Rio:Constructor:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevRio:sadd(SARW..'Rio:Constructor:'..msg.chat_id_,result.id_)
...
DevRio:sadd(SARW..'Rio:Constructor:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:Constructor:'..msg.chat_id_,user)
...
DevRio:sadd(SARW..'Rio:Managers:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:Managers:'..msg.chat_id_,user)
...
DevRio:sadd(SARW..'Rio:Cleaner:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevRio:sadd(SARW..'Rio:Cleaner:'..msg.chat_id_,result.id_)
...
DevRio:sadd(SARW..'Rio:Cleaner:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:Cleaner:'..msg.chat_id_,user)
...
DevRio:sadd(SARW..'Rio:Admins:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:Admins:'..msg.chat_id_,user)
...
DevRio:sadd(SARW..'Rio:VipMem:'..msg.chat_id_,user)
...
DevRio:srem(SARW..'Rio:VipMem:'..msg.chat_id_,user)
...
function promote_by_reply(extra, result, success)
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChatMember?chat_id='..msg.chat_id_..'&user_id='..SARW)
local GetInfo = JSON.decode(Check)
...
SARWTeam = {"ها هلاو","انطق","كول","تفضل","احبك","عمري","لاف"};
send(msg.sender_user_id_, 0,SARWTeam[math.random(#SARWTeam)])
end
--     Source SARW     --
...
DevRio:del(SARW.."Rio:Lock:ProSet"..msg.chat_id_)
...
DevRio:set(SARW.."Rio:Lock:ProSet"..msg.chat_id_,"true")
...
if not RioConstructor(msg) and DevRio:get(SARW.."Rio:Lock:KickBan"..msg.chat_id_) then 
...
ChatKick(result.chat_id_, result.sender_user_id_)
DevRio:sadd(SARW..'Rio:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if not Constructor(msg) and DevRio:get(SARW.."Rio:Lock:KickBan"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,'᥀︙لقد تم تعطيل الطرد والحظر من قبل مالك المجموعه')
return false
end
if result.id_ then
...
ChatKick(msg.chat_id_, result.id_)
DevRio:sadd(SARW..'Rio:Ban:'..msg.chat_id_, result.id_)
...
ChatKick(msg.chat_id_, user)
DevRio:sadd(SARW..'Rio:Ban:'..msg.chat_id_, user)
...
DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_, result.id_)
...
DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_, user)
...
if not RioConstructor(msg) and DevRio:get(SARW.."Rio:Lock:MuteTked"..msg.chat_id_) then 
...
if DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevRio:sadd(SARW..'Rio:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevRio:sadd(SARW..'Rio:Muted:'..msg.chat_id_, result.id_)
...
if DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevRio:sadd(SARW..'Rio:Muted:'..msg.chat_id_, user)
...
if not DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو ليس مكتوم لالغاء كتمه")  
else
DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
if not DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","᥀︙هو ليس مكتوم لالغاء كتمه")  
else
DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_, result.id_)
...
if not DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","᥀︙هو ليس مكتوم لالغاء كتمه")  
else
DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_, user)
...
DevRio:sadd(SARW..'Rio:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevRio:sadd(SARW..'Rio:Tkeed:'..msg.chat_id_, result.id_)
...
DevRio:sadd(SARW..'Rio:Tkeed:'..msg.chat_id_, user)
...
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_, result.id_)
...
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_, user)
...
if DevRio:sismember(SARW..'Rio:RioSudo:',result.sender_user_id_) and not Sudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevRio:sismember(SARW..'Rio:SecondSudo:',result.sender_user_id_) and not RioSudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
ChatKick(result.chat_id_, result.sender_user_id_)
DevRio:sadd(SARW..'Rio:BanAll:', result.sender_user_id_)
...
if DevRio:sismember(SARW..'Rio:RioSudo:',result.id_) and not Sudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevRio:sismember(SARW..'Rio:SecondSudo:',result.id_) and not RioSudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
if result.id_ then
ChatKick(msg.chat_id_, result.id_)
DevRio:sadd(SARW..'Rio:BanAll:', result.id_)
...
if tonumber(user) == tonumber(SARW) then  
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر البوت عام*", 1, 'md')
return false 
end
...
if DevRio:sismember(SARW..'Rio:RioSudo:',user) and not Sudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevRio:sismember(SARW..'Rio:SecondSudo:',user) and not RioSudo(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
ChatKick(msg.chat_id_, user)
DevRio:sadd(SARW..'Rio:BanAll:', user)
...
DevRio:sadd(SARW..'Rio:MuteAll:', result.sender_user_id_)          
...
if result.id_ then
DevRio:sadd(SARW..'Rio:MuteAll:', result.id_)
...
DevRio:sadd(SARW..'Rio:MuteAll:', user)
...
DevRio:srem(SARW..'Rio:BanAll:', result.sender_user_id_)
DevRio:srem(SARW..'Rio:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevRio:srem(SARW..'Rio:BanAll:', result.id_)
DevRio:srem(SARW..'Rio:MuteAll:', result.id_)
...
DevRio:srem(SARW..'Rio:BanAll:', user)
DevRio:srem(SARW..'Rio:MuteAll:', user)
...
DevRio:setex(SARW.."Rio:EditDev"..msg.sender_user_id_,300,true)
end
if DevRio:get(SARW.."Rio:EditDev"..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_,'᥀︙تم الغاء امر تغير المطور الاساسي')
DevRio:del(SARW.."Rio:EditDev"..msg.sender_user_id_)
return false
end
if text and text:match("^(%d+)$") then 
...
if dp.first_name_ ~= false then
DevRio:del(SARW.."Rio:EditDev"..msg.sender_user_id_)
DevRio:set(SARW.."Rio:NewDev"..msg.sender_user_id_,dp.id_)
...
--     Source SARW     --
if msg.reply_to_message_id_ ~= 0 then
if text and text:match("^رفع مطي$") and not DevRio:get(SARW..'Rio:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if DevRio:sismember(SARW..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو مطي شرفع منه بعد😹💔") 
else
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙تم رفعه في قائمة المطايه") 
DevRio:sadd(SARW..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
end end
getMessage(msg.chat_id_, msg.reply_to_message_id_,donky_by_reply)
end end
--     Source SARW     --
if msg.reply_to_message_id_ ~= 0  then
if text and text:match("^تنزيل مطي$") and not DevRio:get(SARW..'Rio:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if not DevRio:sismember(SARW..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو ليس مطي ليتم تنزيله") 
else
DevRio:srem(SARW..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
...
DevRio:set('SARWTeam:'..SARW..'id:user'..msg.chat_id_,TXT)  
DevRio:setex('SARWTeam:'..SARW.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
DevRio:del(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_) 
DevRio:incrby(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_,Num) 
...
DevRio:set('SARWTeam:'..SARW..'ids:user'..msg.chat_id_,TXT)  
DevRio:setex('SARWTeam:'..SARW.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
function Reply(extra, result, success)
DevRio:incrby(SARW..'Rio:GamesNumber'..msg.chat_id_..result.sender_user_id_,Num) 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم اضافة "..Num..' نقطه', 1, 'md') 
end
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},Reply, nil)
return false
end
if DevRio:get(SARW..'Rio:Lock:Clean'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ or msg.content_.animated_ then if msg.reply_to_message_id_ ~= 0 then DevRio:sadd(SARW.."Rio:cleaner"..msg.chat_id_, msg.id_) else DevRio:sadd(SARW.."Rio:cleaner"..msg.chat_id_, msg.id_) end end end
if DevRio:get(SARW..'Rio:Lock:CleanNum'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ or msg.content_.animated_ then if msg.reply_to_message_id_ ~= 0 then DevRio:sadd(SARW.."Rio:cleanernum"..msg.chat_id_, msg.id_) else DevRio:sadd(SARW.."Rio:cleanernum"..msg.chat_id_, msg.id_) end end end
if DevRio:get(SARW..'Rio:Lock:CleanMusic'..msg.chat_id_) then if msg.content_.voice_ or msg.content_.audio_ then if msg.reply_to_message_id_ ~= 0 then DevRio:sadd(SARW.."Rio:cleanermusic"..msg.chat_id_, msg.id_) else DevRio:sadd(SARW.."Rio:cleanermusic"..msg.chat_id_, msg.id_) end end end
if Manager(msg) and msg.reply_to_message_id_ ~= 0 then
if text and text:match("^تثبيت$") and ChCheck(msg) then 
if DevRio:sismember(SARW.."Rio:Lock:Pinpin",msg.chat_id_) and not BasicConstructor(msg) then
...
if data.ID == "Ok" then
DevRio:set(SARW..'Rio:PinnedMsg'..msg.chat_id_,msg.reply_to_message_id_)
...
local List = DevRio:smembers(SARW..'Rio:VipMem:'..msg.chat_id_)
text = "᥀︙قائمة المميزين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..rio)
text = "᥀︙قائمة الادمنيه ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..'Rio:Managers:'..msg.chat_id_)
text = "᥀︙قائمة المدراء ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..'Rio:Cleaner:'..msg.chat_id_)
text = "᥀︙قائمة المنظفين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..'Rio:Constructor:'..msg.chat_id_)
text = "᥀︙قائمة المنشئين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..'Rio:Owner:'..msg.chat_id_)
text = "᥀︙قائمة المالكين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..'Rio:BasicConstructor:'..msg.chat_id_)
...
local List = DevRio:smembers(SARW..'Rio:Muted:'..msg.chat_id_)
...
local List = DevRio:smembers(SARW..'Rio:Tkeed:'..msg.chat_id_)
text = "᥀︙قائمة المقيدين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
local List = DevRio:smembers(SARW..'Rio:Ban:'..msg.chat_id_)
...
local List = DevRio:smembers(SARW..'User:Donky:'..msg.chat_id_)
...
local List = DevRio:hkeys(SARW..'Rio:Filters:'..msg.chat_id_)
text = "᥀︙قائمة المنع ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k, v in pairs(List) do
text = text..k..'~ ❨ '..v..' ❩\n'
end
...
local List = DevRio:smembers(SARW..'Rio:RioSudo:')
...
local List = DevRio:smembers(SARW..'Rio:SecondSudo:')
...
local BanAll = DevRio:smembers(SARW..'Rio:BanAll:')
local MuteAll = DevRio:smembers(SARW..'Rio:MuteAll:')
if #BanAll ~= 0 then 
...
local List = DevRio:smembers(SARW..'Rio:SudoBot:')
text = "᥀︙قائمة المطورين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local sudouser = DevRio:get(SARW..'Rio:Sudos'..v) 
local username = DevRio:get(SARW..'Save:UserName'..v)
if username then
...
DevRio:sadd(SARW.."Rio:RioConstructor:"..msg.chat_id_,dp.id_)
end,nil)   
end,nil)   
end
end 
...
if result.content_.sticker_ then
local idsticker = result.content_.sticker_.sticker_.persistent_id_
DevRio:srem(SARW.."Rio:FilterSteckr"..msg.chat_id_,idsticker)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء منع الملصق يمكنهم ارساله الان', 1, 'md')
return false
end
if result.content_.ID == "MessagePhoto" then
local photo = result.content_.photo_.id_
DevRio:srem(SARW.."Rio:FilterPhoto"..msg.chat_id_,photo)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء منع الصوره يمكنهم ارسالها الان', 1, 'md')
return false
end
if result.content_.animation_.animation_ then
local idanimation = result.content_.animation_.animation_.persistent_id_
DevRio:srem(SARW.."Rio:FilterAnimation"..msg.chat_id_,idanimation)
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Thwel:Rio'..msg.chat_id_) 
end
if text and (text == "تعطيل تحويل الصيغ" or text == "تعطيل التحويل") and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل تحويل الصيغ'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Thwel:Rio'..msg.chat_id_,true)  
end
if text == 'تحويل' and not DevRio:get(SARW..'Rio:Thwel:Rio'..msg.chat_id_) then  
if tonumber(msg.reply_to_message_id_) > 0 then 
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..data.id_) or 0
local user_nkt = tonumber(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..data.id_) or 0)
if DevRio:sismember(SARW..'Rio:BanAll:',result.sender_user_id_) then
Tkeed = 'محظور عام'
elseif DevRio:sismember(SARW..'Rio:MuteAll:',result.sender_user_id_) then
Tkeed = 'مكتوم عام'
elseif DevRio:sismember(SARW..'Rio:Ban:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'محظور'
elseif DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مكتوم'
elseif DevRio:sismember(SARW..'Rio:Tkeed:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevRio:sismember(SARW..'Rio:SudoBot:',result.sender_user_id_) and SudoBot(msg) then
sudobot = '\n᥀︙عدد الكروبات ↫ '..(DevRio:get(SARW..'Rio:Sudos'..result.sender_user_id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..res.id_) or 0
local user_nkt = tonumber(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..res.id_) or 0)
if DevRio:sismember(SARW..'Rio:BanAll:',res.id_) then
Tkeed = 'محظور عام'
elseif DevRio:sismember(SARW..'Rio:MuteAll:',res.id_) then
Tkeed = 'مكتوم عام'
elseif DevRio:sismember(SARW..'Rio:Ban:'..msg.chat_id_,res.id_) then
Tkeed = 'محظور'
elseif DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_,res.id_) then
Tkeed = 'مكتوم'
elseif DevRio:sismember(SARW..'Rio:Tkeed:'..msg.chat_id_,res.id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevRio:sismember(SARW..'Rio:SudoBot:',res.id_) and SudoBot(msg) then
sudobot = '\n᥀︙عدد الكروبات ↫ '..(DevRio:get(SARW..'Rio:Sudos'..res.id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..iduser) or 0
local user_nkt = tonumber(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..iduser) or 0)
if DevRio:sismember(SARW..'Rio:BanAll:',iduser) then
Tkeed = 'محظور عام'
elseif DevRio:sismember(SARW..'Rio:MuteAll:',iduser) then
Tkeed = 'مكتوم عام'
elseif DevRio:sismember(SARW..'Rio:Ban:'..msg.chat_id_,iduser) then
Tkeed = 'محظور'
elseif DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_,iduser) then
Tkeed = 'مكتوم'
elseif DevRio:sismember(SARW..'Rio:Tkeed:'..msg.chat_id_,iduser) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevRio:sismember(SARW..'Rio:SudoBot:',iduser) and SudoBot(msg) then
sudobot = '\n᥀︙عدد الكروبات ↫ '..(DevRio:get(SARW..'Rio:Sudos'..iduser) or 0)..'' 
else 
sudobot = '' 
end
...
if DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_,result.sender_user_id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevRio:sismember(SARW..'Rio:Ban:'..msg.chat_id_,result.sender_user_id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevRio:sismember(SARW..'Rio:BanAll:',result.sender_user_id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevRio:sismember(SARW..'Rio:MuteAll:',result.sender_user_id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevRio:sismember(SARW..'Rio:Tkeed:',result.sender_user_id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
...
if result.id_ then
if DevRio:sismember(SARW..'Rio:Muted:'..msg.chat_id_,result.id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevRio:sismember(SARW..'Rio:Ban:'..msg.chat_id_,result.id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevRio:sismember(SARW..'Rio:BanAll:',result.id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevRio:sismember(SARW..'Rio:MuteAll:',result.id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevRio:sismember(SARW..'Rio:Tkeed:',result.id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙الحظر العام ↫ '..banall..'\n᥀︙الكتم العام ↫ '..muteall..'\n᥀︙الحظر ↫ '..banned..'\n᥀︙الكتم ↫ '..muted..'\n᥀︙التقيد ↫ '..tkeed, 1, 'md')  
else
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙*المعرف غير صحيح*', 1, 'md')  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_,result.sender_user_id_) DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_,result.sender_user_id_) DevRio:srem(SARW..'Rio:BanAll:',result.sender_user_id_) DevRio:srem(SARW..'Rio:MuteAll:',result.sender_user_id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_,result.sender_user_id_) DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_,result.sender_user_id_) 
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),unbanreply) 
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_,user) DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_,user) DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_,user) DevRio:srem(SARW..'Rio:BanAll:',user) DevRio:srem(SARW..'Rio:MuteAll:',user)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_,user) DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_,user) DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_,user) 
end  
end,nil)  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_,result.id_) DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_,result.id_) DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_,result.id_) DevRio:srem(SARW..'Rio:BanAll:',result.id_) DevRio:srem(SARW..'Rio:MuteAll:',result.id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_,result.id_) DevRio:srem(SARW..'Rio:Ban:'..msg.chat_id_,result.id_) DevRio:srem(SARW..'Rio:Muted:'..msg.chat_id_,result.id_) 
end
end,nil)   
end  
...
DevRio:set(SARW.."Rio:GpIds:Text"..msg.chat_id_,Text_Rand)
...
DevRio:set("SARW:New:id:"..SARW..msg.sender_user_id_,'SARWTeam')
return "SARWTeam"
end
if text and DevRio:get("SARW:New:id:"..SARW..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevRio:del("SARW:New:id:"..SARW..msg.sender_user_id_)
return false
end
DevRio:del("SARW:New:id:"..SARW..msg.sender_user_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ كليشة الايدي العامه', 1, 'md')
DevRio:set(SARW.."Rio:AllIds:Text",text)
return false
end
...
DevRio:del(SARW.."Rio:AllIds:Text")
end
end
--     Source SARW     --
if text and text:match("^تعيين الايدي$") and ChCheck(msg) or text and text:match("^تعين الايدي$") and ChCheck(msg) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙رجائا اتبع التعليمات للتعيين \n᥀︙لطبع كليشة الايدي ارسل كليشه تحتوي على النصوص التي باللغه الانجليزيه ادناه ↫ ⤈\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n `#username` ↬ لطبع المعرف\n `#id` ↬ لطبع الايدي \n `#photos` ↬ لطبع عدد الصور \n `#stast` ↬ لطبع الرتب \n `#msgs` ↬ لطبع عدد الرسائل \n `#msgday` ↬ لطبع الرسائل اليوميه \n `#CustomTitle` ↬ لطبع اللقب \n `#bio` ↬ لطبع البايو \n `#auto` ↬ لطبع التفاعل \n `#game` ↬ لطبع عدد النقاط \n `#cont` ↬ لطبع عدد الجهات \n `#edit` ↬ لطبع عدد السحكات \n `#Description` ↬ لطبع تعليق الصور\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉', 1, 'md')
DevRio:set("SARW:New:id:"..SARW..msg.chat_id_..msg.sender_user_id_,'SARWTeam')
return "SARWTeam"
end
if text and Manager(msg) and DevRio:get("SARW:New:id:"..SARW..msg.chat_id_..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevRio:del("SARW:New:id:"..SARW..msg.chat_id_..msg.sender_user_id_)
return false
end
DevRio:del("SARW:New:id:"..SARW..msg.chat_id_..msg.sender_user_id_)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ الكليشه الجديده', 1, 'md')
DevRio:set(SARW.."Rio:GpIds:Text"..msg.chat_id_,text)
return false
end
if text and text:match("^حذف الايدي$") and ChCheck(msg) or text and text:match("^مسح الايدي$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف كليشة الايدي")  
DevRio:del(SARW.."Rio:GpIds:Text"..msg.chat_id_)
end
end
--     Source SARW     --
if msg.reply_to_message_id_ ~= 0 then
...
function SARWTeam(extra,rio,success)
...
local msgsday = DevRio:get(SARW..'Rio:UsersMsgs'..SARW..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit_msg = DevRio:get(SARW..'Rio:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevRio:get(SARW..'Rio:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local user_nkt = tonumber(DevRio:get(SARW..'Rio:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
local cont = (tonumber(DevRio:get(SARW..'Rio:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
if not DevRio:get(SARW..'Rio:Lock:Id'..msg.chat_id_) then 
if not DevRio:get(SARW..'Rio:Lock:Id:Photo'..msg.chat_id_) then 
if DevRio:get(SARW.."Rio:AllIds:Text") then
newpicid = DevRio:get(SARW.."Rio:AllIds:Text")
...
if not DevRio:get(SARW.."Rio:GpIds:Text"..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, rio.photos_[0].sizes_[1].photo_.persistent_id_,newpicid,msg.id_,msg.id_.."")
else 
local new_id = DevRio:get(SARW.."Rio:GpIds:Text"..msg.chat_id_)
...
if DevRio:get(SARW.."Rio:AllIds:Text") then
newallid = DevRio:get(SARW.."Rio:AllIds:Text")
...
if DevRio:get(SARW.."Rio:AllIds:Text") then
notpicid = DevRio:get(SARW.."Rio:AllIds:Text")
...
if not DevRio:get(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التعديل")  
DevRio:set(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_,true)
else
...
DevRio:set(SARW.."Rio:Lock:Bots"..msg.chat_id_,"ked")  
end
if LockText[2] == "التكرار" or LockText[2] == "التكرار بالحذف" then 
DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User","del")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التكرار بالحذف")  
end
if LockText[2] == "التكرار بالطرد" then 
DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User","kick")  
...
DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التكرار بالتقيد")  
end
if LockText[2] == "التكرار بالكتم" then 
DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User","mute")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التكرار بالكتم")  
end
if BasicConstructor(msg) then
if LockText[2] == "التثبيت" then
if not DevRio:get(SARW..'Rio:Lock:Pin'..msg.chat_id_) then
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  DevRio:set(SARW.."Rio:PinnedMsg"..msg.chat_id_,data.pinned_message_id_)  end,nil)
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التثبيت")  
DevRio:set(SARW..'Rio:Lock:Pin'..msg.chat_id_,true)
DevRio:sadd(SARW.."Rio:Lock:Pinpin",msg.chat_id_) 
else
...
DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Num:Spam" ,TextSpam) 
end
end
if text and (text:match("^ضع زمن التكرار (%d+)$") or text:match("^وضع زمن التكرار (%d+)$")) then  
local TextSpam = text:match("ضع زمن التكرار (%d+)$") or text:match("وضع زمن التكرار (%d+)$")
DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Num:Spam:Time" ,TextSpam) 
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:Id:Photo'..msg.chat_id_)
end end
if text and text == 'تعطيل الايدي بالصوره' and ChCheck(msg) then
if DevRio:get(SARW..'Rio:Lock:Id:Photo'..msg.chat_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙الايدي بالصوره بالتاكيد معطل', 1, 'md')
else
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الايدي بالصوره'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:Id:Photo'..msg.chat_id_,true)
end end 
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:Id'..msg.chat_id_)
end end 
if text and text == 'تعطيل الايدي' and ChCheck(msg) then
if DevRio:get(SARW..'Rio:Lock:Id'..msg.chat_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙الايدي بالتاكيد معطل ', 1, 'md')
else
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الايدي بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:Id'..msg.chat_id_,true)
end end
end
--     Source SARW     --
...
DevRio:setex(SARW.."Rio:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_,300,true) 
end
if text == 'انشاء رابط' and ChCheck(msg) or text == 'انشاء الرابط' and ChCheck(msg) then
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_))
if not DevRio:get(SARW.."Rio:Groups:Links"..msg.chat_id_)  then 
if LinkGp.ok == true then 
LinkGroup = LinkGp.result
DevRio:set(SARW.."Rio:Groups:Links"..msg.chat_id_,LinkGroup) 
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW.."Rio:Lock:Welcome"..msg.chat_id_,true)
end
if text and text:match("^تعطيل الترحيب$") and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الترحيب بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW.."Rio:Lock:Welcome"..msg.chat_id_)
end
if DevRio:get(SARW..'Rio:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الترحيب', 1, 'md')
DevRio:del(SARW..'Rio:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevRio:del(SARW..'Rio:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
Welcomes = text:gsub('"',"") Welcomes = text:gsub("'","") Welcomes = text:gsub(",","") Welcomes = text:gsub("*","") Welcomes = text:gsub(";","") Welcomes = text:gsub("`","") Welcomes = text:gsub("{","") Welcomes = text:gsub("}","") 
DevRio:set(SARW..'Rio:Groups:Welcomes'..msg.chat_id_,Welcomes)
...
DevRio:set(SARW..'Rio:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
if text and text:match("^حذف الترحيب$") and ChCheck(msg) or text and text:match("^حذف ترحيب$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف الترحيب")  
DevRio:del(SARW..'Rio:Groups:Welcomes'..msg.chat_id_)
end
if text and text:match("^جلب الترحيب$") and ChCheck(msg) or text and text:match("^جلب ترحيب$") and ChCheck(msg) or text and text:match("^الترحيب$") and ChCheck(msg) then
local Welcomes = DevRio:get(SARW..'Rio:Groups:Welcomes'..msg.chat_id_)
if Welcomes then
Dev_Rio(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
else
...
--     Source SARW     --
if DevRio:get(SARW..'Rio:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_) then  
if text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء حفظ الوصف", 1, 'md')
DevRio:del(SARW..'Rio:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevRio:del(SARW..'Rio:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevRio:set(SARW..'Rio:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
DevRio:set(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_,"add")  
return false  
end    
if DevRio:get(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "add" then
if text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر المنع', 1, 'md')
DevRio:del(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم منع الكلمه ↫ "..text, 1, 'html')
DevRio:del(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
DevRio:hset(SARW..'Rio:Filters:'..msg.chat_id_, text,'newword')
return false
end
if text and text == "الغاء منع" and msg.reply_to_message_id_ == 0 and ChCheck(msg) then       
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙ارسل لي الكلمه الان", 1, 'md') 
DevRio:set(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_,"del")  
return false  
end    
if DevRio:get(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "del" then   
if text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر الغاء المنع', 1, 'md')
DevRio:del(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
if not DevRio:hget(SARW..'Rio:Filters:'..msg.chat_id_, text) then  
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙الكلمه ↫ "..text.." غير ممنوعه", 1, 'html')
DevRio:del(SARW.."Rio:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
else
DevRio:hdel(SARW..'Rio:Filters:'..msg.chat_id_, text)
...
local gps = DevRio:scard(SARW.."Rio:Groups") local users = DevRio:scard(SARW.."Rio:Users") 
...
local users = DevRio:scard(SARW.."Rio:Users")
...
local gps = DevRio:scard(SARW.."Rio:Groups")
...
if not DevRio:get(SARW..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_) then  
...
DevRio:setex(SARW..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_,300,true)
end 
end
...
local pv = DevRio:smembers(SARW.."Rio:Users")
local sendok = 0
for i = 1, #pv do
...
DevRio:srem(SARW.."Rio:Users",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = DevRio:smembers(SARW.."Rio:Groups")
local w = 0
local q = 0
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
DevRio:srem(SARW.."Rio:Groups",group[i]) 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = group[i], user_id_ = SARW, status_ = { ID = "ChatMemberStatusLeft" }, }, dl_cb, nil)
w = w + 1
end
...
Dev_Rio(msg.chat_id_, msg.id_, 1,'᥀︙عدد الكروبات الان ↫ { '..#group..' }'..SARWgp1..SARWgp2..'\n᥀︙العدد الحقيقي الان  ↫ ( '..SARWgp3..' ) \n ', 1, 'md')
end end
end,nil)
end
return false
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:CleanNum'..msg.chat_id_,true)  
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:CleanNum'..msg.chat_id_) 
end
...
DevRio:set(SARW..'Rio:CleanNum'..msg.chat_id_,Num) 
end end 
if msg and DevRio:get(SARW..'Rio:Lock:CleanNum'..msg.chat_id_) then
if DevRio:get(SARW..'Rio:CleanNum'..msg.chat_id_) then CleanNum = DevRio:get(SARW..'Rio:CleanNum'..msg.chat_id_) else CleanNum = 200 end
if DevRio:scard(SARW.."Rio:cleanernum"..msg.chat_id_) >= tonumber(CleanNum) then 
local List = DevRio:smembers(SARW.."Rio:cleanernum"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevRio:del(SARW.."Rio:cleanernum"..msg.chat_id_)
end 
end
if CleanerNum(msg) then
if DevRio:get(SARW..'Rio:Lock:CleanNum'..msg.chat_id_) then 
if text == "التلقائي" and ChCheck(msg) or text == "عدد التلقائي" and ChCheck(msg) then 
local M = DevRio:scard(SARW.."Rio:cleanernum"..msg.chat_id_)
if M ~= 0 then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙عدد الميديا ↫ "..M.."\n᥀︙الحذف التلقائي ↫ "..(DevRio:get(SARW..'Rio:CleanNum'..msg.chat_id_) or 200), 1, 'md') 
else
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل امسح بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:Clean'..msg.chat_id_,true)  
end
if text == "تعطيل امسح" and Constructor(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل امسح بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:Clean'..msg.chat_id_) 
end
if Cleaner(msg) then
if DevRio:get(SARW..'Rio:Lock:Clean'..msg.chat_id_) then 
if text == "الميديا" and ChCheck(msg) or text == "عدد الميديا" and ChCheck(msg) then 
local M = DevRio:scard(SARW.."Rio:cleaner"..msg.chat_id_)
if M ~= 0 then
...
DevRio:del(SARW.."Rio:cleaner"..msg.chat_id_)
else
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙لاتوجد ميديا هنا", 1, 'md') 
end end 
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:CleanMusic'..msg.chat_id_,true)  
end
if text == "تعطيل مسح الاغاني" and Constructor(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل مسح الاغاني بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:CleanMusic'..msg.chat_id_) 
end
if CleanerMusic(msg) then
if DevRio:get(SARW..'Rio:Lock:CleanMusic'..msg.chat_id_) then 
if text == "الاغاني" and ChCheck(msg) or text == "عدد الاغاني" and ChCheck(msg) then 
local M = DevRio:scard(SARW.."Rio:cleanermusic"..msg.chat_id_)
if M ~= 0 then
...
local List = DevRio:smembers(SARW.."Rio:cleanermusic"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevRio:del(SARW.."Rio:cleanermusic"..msg.chat_id_)
else
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙لاتوجد اغاني هنا", 1, 'md') 
end end end end
--     Source SARW     --
...
if DevRio:get(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح التعديل")  
DevRio:del(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_)
else
...
DevRio:del(SARW.."Rio:Lock:Bots"..msg.chat_id_)  
end end
if UnLockText[2] == "التكرار" then 
DevRio:hdel(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح التكرار")  
end
if BasicConstructor(msg) then
if UnLockText[2] == "التثبيت" then
if DevRio:get(SARW..'Rio:Lock:Pin'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح التثبيت")  
DevRio:del(SARW..'Rio:Lock:Pin'..msg.chat_id_)
DevRio:srem(SARW.."Rio:Lock:Pinpin",msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Text'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Photo'..msg.chat_id_)
else
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙الصور بالفعل مفتوحه في المجموعه', 1, 'md')
end
end
if UnLockText[2] == "الكلايش" then
if DevRio:get(SARW..'Rio:Lock:Spam'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح الكلايش")  
DevRio:del(SARW..'Rio:Lock:Spam'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Join'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Videos'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Document'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Inline'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Markdown'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Gifs'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Music'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Voice'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Links'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Location'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Tags'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Hashtak'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Contact'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:WebLinks'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Arabic'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:English'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:TagServr'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Stickers'..msg.chat_id_)
else
...
DevRio:del(SARW..'Rio:Lock:Forwards'..msg.chat_id_)
else
...
DevRio:set(SARW.."Rio:Lock:Bots"..msg.chat_id_,"del") DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Rio:Lock:Links','Rio:Lock:Contact','Rio:Lock:Forwards','Rio:Lock:Videos','Rio:Lock:Gifs','Rio:Lock:EditMsgs','Rio:Lock:Stickers','Rio:Lock:Farsi','Rio:Lock:Spam','Rio:Lock:WebLinks','Rio:Lock:Photo'}
for i,Lock in pairs(LockList) do
DevRio:set(SARW..Lock..msg.chat_id_,true)
end
...
DevRio:del(SARW..UnLock..msg.chat_id_)
end
...
DevRio:del(SARW..'Rio:Lock:Fshar'..msg.chat_id_) DevRio:del(SARW..'Rio:Lock:Taf'..msg.chat_id_) DevRio:del(SARW..'Rio:Lock:Kfr'..msg.chat_id_) 
DevRio:set(SARW.."Rio:Lock:Bots"..msg.chat_id_,"del") DevRio:hset(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
...
DevRio:set(SARW..'Rio:Lock:Fshar'..msg.chat_id_,true) DevRio:set(SARW..'Rio:Lock:Taf'..msg.chat_id_,true) DevRio:set(SARW..'Rio:Lock:Kfr'..msg.chat_id_,true) DevRio:hdel(SARW.."Rio:Spam:Group:User"..msg.chat_id_ ,"Spam:User") 
...
DevRio:set(SARW..'Rio:Spam:Text'..msg.chat_id_,SetSpam)
...
DevRio:set(SARW.."Rio:SudoBot:Rd"..msg.chat_id_,Text)
...
DevRio:set(SARW.."Rio:BasicConstructor:Rd"..msg.chat_id_,Text)
...
DevRio:set(SARW.."Rio:Constructor:Rd"..msg.chat_id_,Text)
...
DevRio:set(SARW.."Rio:Managers:Rd"..msg.chat_id_,Text) 
...
DevRio:set(SARW.."Rio:Admins:Rd"..msg.chat_id_,Text)
...
DevRio:set(SARW.."Rio:VipMem:Rd"..msg.chat_id_,Text)
...
DevRio:set(SARW.."Rio:Cleaner:Rd"..msg.chat_id_,Text)
...
DevRio:set(SARW.."Rio:mem:Rd"..msg.chat_id_,Text)
...
DevRio:del(SARW.."Rio:mem:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:Cleaner:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:VipMem:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:Admins:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:Managers:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:Constructor:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:BasicConstructor:Rd"..msg.chat_id_)
DevRio:del(SARW.."Rio:SudoBot:Rd"..msg.chat_id_)
end
end
--     Source SARW     --
...
if tonumber(admins[i].user_id_) ~= tonumber(SARW) then
...
DevRio:del(SARW..'Rio:Owner:'..msg.chat_id_)
end
end
if Owner(msg) then
if txts[2] == 'المنشئين الاساسيين' or txtss[2] == 'المنشئين الاساسيين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف المنشئين الاساسيين")  
DevRio:del(SARW..'Rio:BasicConstructor:'..msg.chat_id_)
end
end
if BasicConstructor(msg) then
if txts[2] == 'المنشئين' or txtss[2] == 'المنشئين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف المنشئين")  
DevRio:del(SARW..'Rio:Constructor:'..msg.chat_id_)
end end
if Constructor(msg) then
if txts[2] == 'المدراء' or txtss[2] == 'المدراء' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف المدراء")  
DevRio:del(SARW..'Rio:Managers:'..msg.chat_id_)
end 
...
DevRio:srem(SARW..'Rio:Tkeed:'..msg.chat_id_, v)
end 
...
DevRio:del(SARW..'Rio:Filters:'..msg.chat_id_)
DevRio:del(SARW.."Rio:FilterAnimation"..msg.chat_id_)
DevRio:del(SARW.."Rio:FilterPhoto"..msg.chat_id_)
DevRio:del(SARW.."Rio:FilterSteckr"..msg.chat_id_)
end
...
DevRio:del(SARW..'Rio:Ban:'..msg.chat_id_) DevRio:del(SARW..'Rio:Admins:'..msg.chat_id_) DevRio:del(SARW..'User:Donky:'..msg.chat_id_) DevRio:del(SARW..'Rio:VipMem:'..msg.chat_id_) DevRio:del(SARW..'Rio:Filters:'..msg.chat_id_) DevRio:del(SARW..'Rio:Muted:'..msg.chat_id_)
...
local basicconstructor = DevRio:smembers(SARW..'Rio:BasicConstructor:'..msg.chat_id_)
local constructor = DevRio:smembers(SARW..'Rio:Constructor:'..msg.chat_id_)
local Managers = DevRio:smembers(SARW..'Rio:Managers:'..msg.chat_id_)
local admins = DevRio:smembers(SARW..'Rio:Admins:'..msg.chat_id_)
local vipmem = DevRio:smembers(SARW..'Rio:VipMem:'..msg.chat_id_)
local donky = DevRio:smembers(SARW..'User:Donky:'..msg.chat_id_)
...
spam_c = DevRio:get(SARW..'Rio:Spam:Text'..msg.chat_id_)
end
--     Source SARW     --
if DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "بالطرد"     
elseif DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "بالتقيد"     
elseif DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "بالكتم"           
elseif DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "بالحذف"
else     
flood = "مفتوح"     
end
--     Source SARW     --
if DevRio:get(SARW.."Rio:Lock:Bots"..msg.chat_id_) == "del" then
lock_bots = "بالحذف"
elseif DevRio:get(SARW.."Rio:Lock:Bots"..msg.chat_id_) == "ked" then
lock_bots = "بالتقيد"   
elseif DevRio:get(SARW.."Rio:Lock:Bots"..msg.chat_id_) == "kick" then
lock_bots = "بالطرد"    
else
lock_bots = "مفتوحه"    
end
--     Source SARW     --
if DevRio:get(SARW..'Rio:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه'end
if DevRio:get(SARW..'Rio:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفله' else mute_photo = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفله' else mute_video = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفله' else mute_gifs = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Music'..msg.chat_id_) then mute_music = 'مقفله' else mute_music = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفله' else mute_in = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفله' else mute_voice = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفله' else mute_edit = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفله' else lock_pin = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفله' else lock_sticker = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفله' else lock_tgservice = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفله' else lock_wp = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفله' else lock_htag = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفله' else lock_tag = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Location'..msg.chat_id_) then lock_location = 'مقفله' else lock_location = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفله' else lock_contact = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:English'..msg.chat_id_) then lock_english = 'مقفله' else lock_english = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفله' else lock_arabic = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفله' else lock_forward = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Document'..msg.chat_id_) then lock_file = 'مقفله' else lock_file = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفله' else markdown = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفله' else lock_spam = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفل' else lock_Join = 'مفتوح' end
if DevRio:get(SARW.."Rio:Lock:Welcome"..msg.chat_id_) then send_welcome = 'مقفله' else send_welcome = 'مفتوحه' end
if DevRio:get(SARW..'Rio:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end
if DevRio:get(SARW..'Rio:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end
if DevRio:get(SARW..'Rio:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end
if DevRio:get(SARW..'Rio:Lock:Farsi'..msg.chat_id_) then lock_farsi = 'مقفله' else lock_farsi = 'مفتوحه' end
local Flood_Num = DevRio:hget(SARW.."Rio:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
--     Source SARW     --
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل ميزة انطق'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Antk:Rio'..msg.chat_id_) 
end
if text == "تعطيل انطق" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل ميزة انطق'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Antk:Rio'..msg.chat_id_,true)  
end
if text and text:match("^انطق (.*)$") and not DevRio:get(SARW..'Rio:Antk:Rio'..msg.chat_id_) and ChCheck(msg) then
...
--     Source SARW     --
if DevRio:get(SARW..'Rio:setrules'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ قوانين المجموعه', 1, 'md')
DevRio:del(SARW..'Rio:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevRio:del(SARW..'Rio:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:rules'..msg.chat_id_,text)
...
DevRio:set(SARW..'Rio:setrules'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
end
if text and text:match("^القوانين$") and ChCheck(msg) then
local rules = DevRio:get(SARW..'Rio:rules'..msg.chat_id_)
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Zrf:Rio'..msg.chat_id_) 
end
if text == "تعطيل الزخرفه" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الزخرفه بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Zrf:Rio'..msg.chat_id_,true)  
end
if DevRio:get(SARW..'Zrf:Rio'..msg.chat_id_..msg.sender_user_id_) then 
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر الزخرفه', 1, 'md')
DevRio:del(SARW..'Zrf:Rio'..msg.chat_id_..msg.sender_user_id_)
return false  
end 
...
if not DevRio:get(SARW..'Rio:Zrf:Rio'..msg.chat_id_) then
if text == 'زخرفه' and ChCheck(msg) or text == 'الزخرفه' and ChCheck(msg) then  
DevRio:setex(SARW.."Zrf:Rio"..msg.chat_id_..msg.sender_user_id_,300,true)
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Brg:Rio'..msg.chat_id_) 
end
if text == "تعطيل الابراج" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الابراج بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Brg:Rio'..msg.chat_id_,true)  
end
if not DevRio:get(SARW..'Rio:Brg:Rio'..msg.chat_id_) then
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Nsba:Rio'..msg.chat_id_) 
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Nsba:Rio'..msg.chat_id_,true)  
end
if not DevRio:get(SARW..'Rio:Nsba:Rio'..msg.chat_id_) then
if text == "نسبه الحب" and ChCheck(msg) or text == "نسبة الحب" and ChCheck(msg) then
DevRio:set(SARW..'LoveNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسمين لحساب نسبة الحب بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الحب" and text ~= "نسبة الحب" and DevRio:get(SARW..'LoveNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الحب ', 1, 'md')
DevRio:del(SARW..'LoveNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
...
DevRio:set(SARW..'RyNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true)
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسمين لحساب نسبة الخيانه بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الخيانه" and text ~= "نسبة الخيانه" and text ~= "↫ نسبه الخيانه ᥀" and DevRio:get(SARW..'RyNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الخيانه ', 1, 'md')
DevRio:del(SARW..'RyNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Rio = math.random(0,100);
...
DevRio:set(SARW..'JNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة جماله كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الجمال" and text ~= "نسبة الجمال" and text ~= "↫ نسبه الجمال ᥀" and DevRio:get(SARW..'JNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الجمال ', 1, 'md')
DevRio:del(SARW..'JNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Rio = math.random(0,100);
...
DevRio:set(SARW..'HataNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسمين لحساب نسبة الكره بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الكره" and text ~= "نسبة الكره" and text ~= "↫ نسبه الكره ᥀" and DevRio:get(SARW..'HataNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الكره ', 1, 'md')
DevRio:del(SARW..'HataNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Rio = math.random(0,100);
...
DevRio:set(SARW..'RjolaNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة رجولته كمثال ↫ جاك', 1, 'md')
end
end
if text and text ~= "نسبه الرجوله" and text ~= "نسبة الرجوله" and text ~= "نسبه رجوله" and text ~= "نسبة رجوله" and text ~= "↫ نسبه الرجوله ᥀" and DevRio:get(SARW..'RjolaNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الرجوله ', 1, 'md')
DevRio:del(SARW..'RjolaNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Rio = math.random(0,100);
...
DevRio:set(SARW..'AnothaNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة انوثته كمثال ↫ روز', 1, 'md')
end
end
if text and text ~= "نسبه الانوثه" and text ~= "نسبة الانوثه" and text ~= "نسبه انوثه" and text ~= "نسبة انوثه" and text ~= "↫ نسبه الانوثه ᥀" and DevRio:get(SARW..'AnothaNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الانوثه ', 1, 'md')
DevRio:del(SARW..'AnothaNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Rio = math.random(0,100);
...
DevRio:set(SARW..'StupidNsba:Rio'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة غبائه كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الغباء" and text ~= "نسبة الغباء" and text ~= "↫ نسبه الغباء ᥀" and DevRio:get(SARW..'StupidNsba:Rio'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الغباء ', 1, 'md')
DevRio:del(SARW..'StupidNsba:Rio'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Rio = math.random(0,100);
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل حساب العمر'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Age:Rio'..msg.chat_id_) 
end
if text == "تعطيل حساب العمر" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل حساب العمر'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Age:Rio'..msg.chat_id_,true)  
end
if not DevRio:get(SARW..'Rio:Age:Rio'..msg.chat_id_) then
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Mean:Rio'..msg.chat_id_) 
end
if text == "تعطيل معاني الاسماء" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل معاني الاسماء'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Mean:Rio'..msg.chat_id_,true)  
end
if not DevRio:get(SARW..'Rio:Mean:Rio'..msg.chat_id_) then
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل المتحركه'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:gif:Rio'..msg.chat_id_) 
end
if text == "تعطيل متحركه" and Manager(msg) and ChCheck(msg) or text == "تعطيل المتحركه" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل المتحركه'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:gif:Rio'..msg.chat_id_,true)  
end
if text and (text == "متحركه" or text == "↫ متحركه ᥀") and not DevRio:get(SARW..'Rio:gif:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(2,1075); 
local Text ='*᥀︙تم اختيار المتحركه لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/gif:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الميمز'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:memz:Rio'..msg.chat_id_) 
end
if text == "تعطيل ميمز" and Manager(msg) and ChCheck(msg) or text == "تعطيل الميمز" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الميمز'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:memz:Rio'..msg.chat_id_,true)  
end
if text and (text == "ميمز" or text == "↫ ميمز ᥀") and not DevRio:get(SARW..'Rio:memz:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(2,1201); 
local Text ='*᥀︙تم اختيار مقطع الميمز لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/memz:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل غنيلي'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Audios:Rio'..msg.chat_id_) 
end
if text == "تعطيل غنيلي" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل غنيلي'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Audios:Rio'..msg.chat_id_,true)  
end
if text and (text == "غنيلي" or text == "↫ غنيلي ᥀") and not DevRio:get(SARW..'Rio:Audios:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(4,2824); 
local Text ='*᥀︙تم اختيار المقطع الصوتي لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/Song:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الاغاني'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:mp3:Rio'..msg.chat_id_) 
end
if text == "تعطيل الاغاني" and Manager(msg) and ChCheck(msg) or text == "تعطيل اغنيه" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الاغاني'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:mp3:Rio'..msg.chat_id_,true)  
end
if text and (text == "اغنيه" or text == "↫ اغنيه ᥀" or text == "اغاني") and not DevRio:get(SARW..'Rio:mp3:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(2,1167); 
local Text ='*᥀︙تم اختيار الاغنيه لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/mp3:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الريمكس'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Remix:Rio'..msg.chat_id_) 
end
if text == "تعطيل ريمكس" and Manager(msg) and ChCheck(msg) or text == "تعطيل الريمكس" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الريمكس'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Remix:Rio'..msg.chat_id_,true)  
end
if text and (text == "ريمكس" or text == "↫ ريمكس ᥀") and not DevRio:get(SARW..'Rio:Remix:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(2,612); 
local Text ='*᥀︙تم اختيار الريمكس لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/remix:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الصوره'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Photo:Rio'..msg.chat_id_) 
end
if text == "تعطيل صوره" and Manager(msg) and ChCheck(msg) or text == "تعطيل الصوره" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الصوره'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Photo:Rio'..msg.chat_id_,true)  
end
if text and (text == "صوره" or text == "↫ صوره ᥀") and not DevRio:get(SARW..'Rio:Photo:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(4,1171); 
local Text ='*᥀︙تم اختيار الصوره لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/photo:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الانمي'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Anime:Rio'..msg.chat_id_) 
end
if text == "تعطيل انمي" and Manager(msg) and ChCheck(msg) or text == "تعطيل الانمي" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الانمي'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Anime:Rio'..msg.chat_id_,true)  
end
if text and (text == "انمي" or text == "↫ انمي ᥀") and not DevRio:get(SARW..'Rio:Anime:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(3,1002); 
local Text ='*᥀︙تم اختيار صورة الانمي لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/anime:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الافلام'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Movies:Rio'..msg.chat_id_) 
end
if text == "تعطيل فلم" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الافلام'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Movies:Rio'..msg.chat_id_,true)  
end
if text and (text == "فلم" or text == "↫ فلم ᥀") and not DevRio:get(SARW..'Rio:Movies:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(45,125); 
local Text ='*᥀︙تم اختيار الفلم لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/Movies:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل المسلسلات'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Series:Rio'..msg.chat_id_) 
end
if text == "تعطيل مسلسل" and Manager(msg) and ChCheck(msg) then
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل المسلسلات'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Series:Rio'..msg.chat_id_,true)  
end
if text and (text == "مسلسل" or text == "↫ مسلسل ᥀") and not DevRio:get(SARW..'Rio:Series:Rio'..msg.chat_id_) and ChCheck(msg) then
Rio = math.random(2,54); 
local Text ='*᥀︙تم اختيار المسلسل لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ᥀",callback_data="/series:"..msg.sender_user_id_}},{{text = '᥀ SARW Team .',url="t.me/YY55c"}}} 
local msg_id = msg.id_/2097152/0.5 
...
--     Source SARW     --
if Admin(msg) then
if DevRio:get(SARW..'Rio:LockSettings'..msg.chat_id_) then 
if text == "الروابط" then if DevRio:get(SARW..'Rio:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الروابط ↫ "..mute_links.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "المعرف" or text == "المعرفات" then if DevRio:get(SARW..'Rio:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفوله' else lock_tag = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙المعرف ↫ "..lock_tag.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "المتحركه" or text == "الملصقات المتحركه" then if DevRio:get(SARW..'Rio:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفوله' else mute_gifs = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙المتحركه ↫ "..mute_gifs.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الملصقات" then if DevRio:get(SARW..'Rio:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفوله' else lock_sticker = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الملصقات ↫ "..lock_sticker.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الصور" then if DevRio:get(SARW..'Rio:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفوله' else mute_photo = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الصور ↫ "..mute_photo.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الفيديو" or text == "الفيديوهات" then if DevRio:get(SARW..'Rio:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفوله' else mute_video = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الفيديو ↫ "..mute_video.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الاونلاين" then if DevRio:get(SARW..'Rio:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفل' else mute_in = 'مفتوح' end local SARWTeam = "\n" .."᥀︙الاونلاين ↫ "..mute_in.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الدردشه" then if DevRio:get(SARW..'Rio:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الدردشه ↫ "..mute_text.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "التوجيه" or text == "اعاده التوجيه" then if DevRio:get(SARW..'Rio:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفل' else lock_forward = 'مفتوح' end local SARWTeam = "\n" .."᥀︙التوجيه ↫ "..lock_forward.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الاغاني" then if DevRio:get(SARW..'Rio:Lock:Music'..msg.chat_id_) then mute_music = 'مقفوله' else mute_music = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الاغاني ↫ "..mute_music.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الصوت" or text == "الصوتيات" then if DevRio:get(SARW..'Rio:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفول' else mute_voice = 'مفتوح' end local SARWTeam = "\n" .."᥀︙الصوت ↫ "..mute_voice.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الجهات" or text == "جهات الاتصال" then if DevRio:get(SARW..'Rio:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفوله' else lock_contact = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الجهات ↫ "..lock_contact.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الماركداون" then if DevRio:get(SARW..'Rio:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفل' else markdown = 'مفتوح' end local SARWTeam = "\n" .."᥀︙الماركداون ↫ "..markdown.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الهاشتاك" then if DevRio:get(SARW..'Rio:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفل' else lock_htag = 'مفتوح' end local SARWTeam = "\n" .."᥀︙الهاشتاك ↫ "..lock_htag.."\n"Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "التعديل" then if DevRio:get(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفل' else mute_edit = 'مفتوح' end local SARWTeam = "\n" .."᥀︙التعديل ↫ "..mute_edit.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "التثبيت" then if DevRio:get(SARW..'Rio:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفل' else lock_pin = 'مفتوح' end local SARWTeam = "\n" .."᥀︙التثبيت ↫ "..lock_pin.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الاشعارات" then if DevRio:get(SARW..'Rio:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفوله' else lock_tgservice = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الاشعارات ↫ "..lock_tgservice.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الكلايش" then if DevRio:get(SARW..'Rio:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفوله' else lock_spam = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الكلايش ↫ "..lock_spam.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الدخول" then if DevRio:get(SARW..'Rio:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفول' else lock_Join = 'مفتوح' end local SARWTeam = "\n" .."᥀︙الدخول ↫ "..lock_Join.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الشبكات" then if DevRio:get(SARW..'Rio:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفوله' else lock_wp = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الشبكات ↫ "..lock_wp.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "المواقع" then if DevRio:get(SARW..'Rio:Lock:Location'..msg.chat_id_) then lock_location = 'مقفوله' else lock_location = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙المواقع ↫ "..lock_location.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "العربيه" then if DevRio:get(SARW..'Rio:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفوله' else lock_arabic = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙العربيه ↫ "..lock_arabic.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الانكليزيه" then if DevRio:get(SARW..'Rio:Lock:English'..msg.chat_id_) then lock_english = 'مقفوله' else lock_english = 'مفتوحه' end local SARWTeam = "\n" .."᥀︙الانكليزيه ↫ "..lock_english.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الكفر" then if DevRio:get(SARW..'Rio:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end local SARWTeam = "\n" .."᥀︙الكفر ↫ "..lock_kaf.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الفشار" then if DevRio:get(SARW..'Rio:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end local SARWTeam = "\n" .."᥀︙الفشار ↫ "..lock_fshar.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
if text == "الطائفيه" then if DevRio:get(SARW..'Rio:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end local SARWTeam = "\n" .."᥀︙الطائفيه ↫ "..lock_taf.."\n" Dev_Rio(msg.chat_id_, msg.id_, 1, SARWTeam, 1, 'md') end
end
--     Source SARW     --
if text == 'تفعيل كشف الاعدادات' and ChCheck(msg) then 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل كشف الاعدادات'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:LockSettings'..msg.chat_id_,true)  
end
if text == 'تعطيل كشف الاعدادات' and ChCheck(msg) then 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل كشف الاعدادات'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:LockSettings'..msg.chat_id_) 
end
--     Source SARW     --
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:Robot'..msg.chat_id_)
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:Robot'..msg.chat_id_,true)
end
--     Source SARW     --
if text == 'تفعيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل ردود المدير'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:GpRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل ردود المدير'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:GpRed'..msg.chat_id_,true)
end
--     Source SARW     --
if text == 'تفعيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل ردود المطور'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:AllRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل ردود المطور'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:AllRed'..msg.chat_id_,true)
end
--     Source SARW     --
if RioSudo(msg) then
...
--     Source SARW     --
if msg.content_.photo_ then
if DevRio:get(SARW..'Rio:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_) then
if msg.content_.photo_.sizes_[3] then
...
DevRio:del(SARW..'Rio:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
send(msg.chat_id_, msg.id_,"᥀︙ليست لدي صلاحية تغير معلومات المجموعه يرجى التحقق من الصلاحيات") 
DevRio:del(SARW..'Rio:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevRio:set(SARW..'Rio:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
--     Source SARW     --
...
if data.ID == "Ok" then
DevRio:del(SARW..'Rio:PinnedMsg'..msg.chat_id_)
...
local PinId = DevRio:get(SARW..'Rio:PinnedMsg'..msg.chat_id_)
if PinId then
...
DevRio:del(SARW..'Rio:Ban:'..msg.chat_id_)
else
local x = 0
...
local Muted = DevRio:scard(SARW.."Rio:Muted:"..msg.chat_id_) or "0"
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
tdcli_function({ID="GetChannelFull",channel_id_ = msg.chat_id_:gsub("-100","")},function(arg,data) 
...
local ConstructorList = DevRio:scard(SARW.."Rio:Constructor:"..ChatId) or 0
local BanedList = DevRio:scard(SARW.."Rio:Ban:"..ChatId) or 0
local ManagerList = DevRio:scard(SARW.."Rio:Managers:"..ChatId) or 0
local MutedList = DevRio:scard(SARW.."Rio:Muted:"..ChatId) or 0
local TkeedList = DevRio:scard(SARW.."Rio:Rio:Tkeed:"..ChatId) or 0
local AdminsList = DevRio:scard(SARW.."Rio:Admins:"..ChatId) or 0
local VipList = DevRio:scard(SARW.."Rio:VipMem:"..ChatId) or 0
...
DevRio:set(SARW..'Rio:Num:Add:Bot',Num) 
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:FreeBot'..SARW) 
end 
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:FreeBot'..SARW,true) 
end 
end
if ChatType == 'sp' or ChatType == 'gp'  then
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Photo:Profile'..msg.chat_id_) 
end
if text == 'تفعيل صورتي' and Manager(msg) and ChCheck(msg) then  
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل صورتي بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Photo:Profile'..msg.chat_id_,true)  
end
if text == 'تفعيل الالعاب' and Manager(msg) and ChCheck(msg) or text == 'تفعيل اللعبه' and Manager(msg) and ChCheck(msg) then   
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل الالعاب بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:Games'..msg.chat_id_) 
end
if text == 'تعطيل الالعاب' and Manager(msg) and ChCheck(msg) or text == 'تعطيل اللعبه' and Manager(msg) and ChCheck(msg) then  
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل الالعاب بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW..'Rio:Lock:Games'..msg.chat_id_,true)  
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW..'Rio:Lock:Gamesinline'..msg.chat_id_) 
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW.."Rio:Lock:GpLinks"..msg.chat_id_,"ok")
return false  
end
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW.."Rio:Lock:GpLinksinline"..msg.chat_id_,"ok")
return false  
end
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW.."Rio:Lock:GpLinksinline"..msg.chat_id_)
return false  
end
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW.."Rio:Lock:GpRd"..msg.chat_id_)
return false  
end
end
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW.."Rio:Lock:GpRd"..msg.chat_id_,true)
return false  
end
end
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تفعيل اضف رد'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW.."Rio:Lock:Rd"..msg.chat_id_)
return false  
end
end
...
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل اضف رد'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW.."Rio:Lock:Rd"..msg.chat_id_,true)
return false  
end
end
--     Source SARW     --
...
Text = "᥀︙تم تفعيل المجموعه "..dp.title_
local inline = {{{text="‹ ترتيب الاوامر ›",callback_data="/SetCmdGp:"..msg.sender_user_id_},{text="‹ رفع الادمنيه ›",callback_data="/UploadAdmin:"..msg.sender_user_id_}},{{text="‹ غادر ›",callback_data="/LeaveBot:"..msg.sender_user_id_},{text="‹ تعطيل ›",callback_data="/Stop:"..msg.sender_user_id_}},{{text="‹ TeAm SARW ›",url="t.me/YY55c"}}}
SendInline(msg.chat_id_,Text,nil,inline,msg.id_/2097152/0.5)
DevRio:sadd(SARW.."Rio:Groups",msg.chat_id_)
if not DevRio:get(SARW..'Rio:SudosGp'..msg.sender_user_id_..msg.chat_id_) and not SecondSudo(msg) then 
DevRio:incrby(SARW..'Rio:Sudos'..msg.sender_user_id_,1)
DevRio:set(SARW..'Rio:SudosGp'..msg.sender_user_id_..msg.chat_id_,"rio")
end
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
if not DevRio:sismember(SARW..'Rio:Groups',msg.chat_id_) then
Dev_Rio(msg.chat_id_, msg.id_, 1, '᥀︙المجموعه بالتاكيد معطله', 1, 'md')
else
Text = "᥀︙تم تعطيل المجموعه "..dp.title_
local inline = {{{text="‹ غادر ›",callback_data="/LeaveBot:"..msg.sender_user_id_},{text="‹ تفعيل ›",callback_data="/On:"..msg.sender_user_id_}},{{text="‹ TeAm SARW ›",url="t.me/YY55c"}}}
SendInline(msg.chat_id_,Text,nil,inline,msg.id_/2097152/0.5)
DevRio:srem(SARW.."Rio:Groups",msg.chat_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
...
local Text = "᥀︙Source SARW\n᥀︙File Bot Groups\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local GroupsManagers = DevRio:scard(SARW.."Rio:Managers:"..v) or 0
local GroupsAdmins = DevRio:scard(SARW.."Rio:Admins:"..v) or 0
local Groupslink = DevRio:get(SARW.."Rio:Groups:Links" ..v)
...
DevRio:setex(SARW.."Rio:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
return false
end 
if DevRio:get(SARW.."Rio:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevRio:del(SARW.."Rio:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevRio:smembers(SARW..'Rio:Users') 
if msg.content_.text_ then
for k,v in pairs(List) do 
RioText = "الرساله"
...
DevRio:setex(SARW.."Rio:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
return false
end 
if DevRio:get(SARW.."Rio:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevRio:del(SARW.."Rio:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevRio:smembers(SARW..'Rio:Groups') 
if msg.content_.text_ then
for k,v in pairs(List) do 
RioText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
...
DevRio:setex(SARW.."Rio:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙ارسل الرساله الان لتوجيها \n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
return false
end 
if DevRio:get(SARW.."Rio:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevRio:del(SARW.."Rio:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevRio:smembers(SARW..'Rio:Groups')   
for k,v in pairs(List) do  
...
DevRio:setex(SARW.."Rio:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙ارسل الرساله الان لتوجيها \n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
return false
end 
if DevRio:get(SARW.."Rio:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevRio:del(SARW.."Rio:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevRio:smembers(SARW..'Rio:Users')   
for k,v in pairs(List) do  
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = msg.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم اذاعة رسالتك بالتوجيه \n᥀︙‏الى ↫ ❨ "..#List.." ❩ مشترك \n ✓", 1, 'md')
...
DevRio:setex(SARW.."Rio:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
return false
end 
if DevRio:get(SARW.."Rio:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == "الغاء" then   
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevRio:del(SARW.."Rio:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
local List = DevRio:smembers(SARW.."Rio:Groups") 
if msg.content_.text_ then
...
RioText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.text_) 
end
elseif msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
RioText = "الصوره"
sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
DevRio:set(SARW..'Rio:PinnedMsgs'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(List) do 
RioText = "المتحركه"
sendDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.video_ then
for k,v in pairs(List) do 
RioText = "الفيديو"
sendVideo(v, 0, 0, 1, nil, msg.content_.video_.video_.persistent_id_,(msg.content_.caption_ or '')) 
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.video_.video_.persistent_id_)
end 
elseif msg.content_.voice_ then
for k,v in pairs(List) do 
RioText = "البصمه"
sendVoice(v, 0, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_,(msg.content_.caption_ or '')) 
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.voice_.voice_.persistent_id_)
end 
elseif msg.content_.audio_ then
for k,v in pairs(List) do 
RioText = "الصوت"
sendAudio(v, 0, 0, 1, nil, msg.content_.audio_.audio_.persistent_id_,(msg.content_.caption_ or '')) 
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.audio_.audio_.persistent_id_)
end 
elseif msg.content_.document_ then
for k,v in pairs(List) do 
RioText = "الملف"
sendDocument(v, 0, 0, 1,nil, msg.content_.document_.document_.persistent_id_,(msg.content_.caption_ or ''))    
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.document_.document_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(List) do 
RioText = "الملصق"
sendSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
DevRio:set(SARW..'Rio:PinnedMsgs'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
...
if not Bot(msg) and DevRio:get(SARW..'Rio:Lock:Rd'..msg.chat_id_) then 
...
local List = DevRio:smembers(SARW..'Rio:Manager:GpRedod'..msg.chat_id_)
if #List == 0 then
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙لا توجد ردود متعدده مضافه" ,  1, "md")
return false
end end
DevRio:set(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedod')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل كلمة الرد اولا" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local DelGpRedRedod = DevRio:get(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
if not DevRio:sismember(SARW..'Rio:Manager:GpRedod'..msg.chat_id_,text) then
...
DevRio:set(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedods')
DevRio:set(SARW..'Rio:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
return false
end end
...
DevRio:set(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedod')
...
DevRio:set(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SetGpRedod')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه الان" ,  1, "md")
return false
end end
if text and text:match("^(.*)$") then
local SetGpRedod = DevRio:get(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
DevRio:set(SARW..'Rio:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SaveGpRedod')
DevRio:set(SARW..'Rio:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevRio:sadd(SARW..'Rio:Manager:GpRedod'..msg.chat_id_,text)
return false
end end
--     Source SARW     --
if text == 'اضف كت' and ChCheck(msg) then
DevRio:set(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_,'SetKt')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل امر `كت تويت` الان" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetKt = DevRio:get(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
if SetKt == 'SetKt' then
if text == "الغاء" then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر" ,  1, "md")
DevRio:del(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
...
DevRio:set(SARW..'Rio:Add:Kt'..msg.sender_user_id_..msg.chat_id_,'SaveKt')
DevRio:set(SARW..'Rio:Add:KtTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevRio:sadd(SARW..'Rio:Sudo:Kt'..msg.chat_id_,text)
return false
end end
--     Source SARW     --
...
DevRio:set(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'DelGpRed')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
...
DevRio:set(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SetGpRed')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end end
if text and text:match("^(.*)$") then
local SetGpRed = DevRio:get(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
...
DevRio:set(SARW..'Rio:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SaveGpRed')
DevRio:set(SARW..'Rio:Add:GpText'..msg.sender_user_id_..msg.chat_id_,text)
DevRio:sadd(SARW..'Rio:Manager:GpRed'..msg.chat_id_,text)
DevRio:set(SARW..'DelManagerRep'..msg.chat_id_,text)
keyboard = {} 
...
DevRio:set(SARW.."Rio:Add:AllRed"..msg.sender_user_id_,'DelAllRed')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
...
DevRio:set(SARW.."Rio:Add:AllRed"..msg.sender_user_id_,'SetAllRed')
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end end
if text and text:match("^(.*)$") then
local SetAllRed = DevRio:get(SARW.."Rio:Add:AllRed"..msg.sender_user_id_)
...
Text = "᥀︙ارسل لي الرد سواء كان ↫ ⤈\n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙يمكنك اضافة الى النص ↫ ⤈\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n `#username` ↬ معرف المستخدم\n `#msgs` ↬ عدد الرسائل\n `#name` ↬ اسم المستخدم\n `#id` ↬ ايدي المستخدم\n `#stast` ↬ رتبة المستخدم\n `#edit` ↬ عدد السحكات\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n᥀︙للخروج ارسل ↫ ( الغاء )\n ✓"
DevRio:set(SARW.."Rio:Add:AllRed"..msg.sender_user_id_,'SaveAllRed')
DevRio:set(SARW.."Rio:Add:AllText"..msg.sender_user_id_, text)
DevRio:sadd(SARW.."Rio:Sudo:AllRed",text)
DevRio:set(SARW.."DelSudoRep",text)
keyboard = {} 
...
local redod = DevRio:smembers(SARW..'Rio:Manager:GpRedod'..msg.chat_id_)
MsgRep = '᥀︙قائمة الردود المتعدده ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
MsgRep = MsgRep..k..'~ (`'..v..'`) • {*العدد ↫ '..#DevRio:smembers(SARW..'Rio:Text:GpTexts'..v..msg.chat_id_)..'*}\n' 
end
if #redod == 0 then
...
if not Bot(msg) and DevRio:get(SARW..'Rio:Lock:GpRd'..msg.chat_id_) then 
...
for k,v in pairs(redod) do
DevRio:del(SARW..'Rio:Text:GpTexts'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Manager:GpRedod'..msg.chat_id_)
end
...
local redod = DevRio:smembers(SARW..'Rio:Manager:GpRed'..msg.chat_id_)
MsgRep = '᥀︙ردود المدير ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
if DevRio:get(SARW.."Rio:Gif:GpRed"..v..msg.chat_id_) then
dp = 'متحركه 🎭'
elseif DevRio:get(SARW.."Rio:Voice:GpRed"..v..msg.chat_id_) then
dp = 'بصمه 🎙'
elseif DevRio:get(SARW.."Rio:Stecker:GpRed"..v..msg.chat_id_) then
dp = 'ملصق 🃏'
elseif DevRio:get(SARW.."Rio:Text:GpRed"..v..msg.chat_id_) then
dp = 'رساله ✉'
elseif DevRio:get(SARW.."Rio:Photo:GpRed"..v..msg.chat_id_) then
dp = 'صوره 🎇'
elseif DevRio:get(SARW.."Rio:Video:GpRed"..v..msg.chat_id_) then
dp = 'فيديو 📽'
elseif DevRio:get(SARW.."Rio:File:GpRed"..v..msg.chat_id_) then
dp = 'ملف 📁'
elseif DevRio:get(SARW.."Rio:Audio:GpRed"..v..msg.chat_id_) then
dp = 'اغنيه 🎶'
end
...
for k,v in pairs(redod) do
DevRio:del(SARW..'Rio:Gif:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Voice:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Audio:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Photo:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Stecker:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Video:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:File:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Text:GpRed'..v..msg.chat_id_)
DevRio:del(SARW..'Rio:Manager:GpRed'..msg.chat_id_)
end
...
MsgRep = '᥀︙ردود المطور ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
if DevRio:get(SARW.."Rio:Gif:AllRed"..v) then
dp = 'متحركه 🎭'
elseif DevRio:get(SARW.."Rio:Voice:AllRed"..v) then
dp = 'بصمه 🎙'
elseif DevRio:get(SARW.."Rio:Stecker:AllRed"..v) then
dp = 'ملصق 🃏'
elseif DevRio:get(SARW.."Rio:Text:AllRed"..v) then
dp = 'رساله ✉'
elseif DevRio:get(SARW.."Rio:Photo:AllRed"..v) then
dp = 'صوره 🎇'
elseif DevRio:get(SARW.."Rio:Video:AllRed"..v) then
dp = 'فيديو 📽'
elseif DevRio:get(SARW.."Rio:File:AllRed"..v) then
dp = 'ملف 📁'
elseif DevRio:get(SARW.."Rio:Audio:AllRed"..v) then
dp = 'اغنيه 🎶'
end
MsgRep = MsgRep..k..'~ (`'..v..'`) ↫ {*'..dp..'*}\n' 
end
...
DevRio:set(SARW..'Rio:NameBot'..msg.sender_user_id_, 'msg')
return false 
end
end
...
DevRio:set(SARW..'Rio:Help0'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help0'..msg.sender_user_id_)
if SARWTeam == 'msg' then
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help0'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevRio:set(SARW..'Rio:Help01'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help01'..msg.sender_user_id_)
if SARWTeam == 'msg' then 
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help01'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help1', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevRio:set(SARW..'Rio:Help21'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help21'..msg.sender_user_id_)
if SARWTeam == 'msg' then
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help21'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help2', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevRio:set(SARW..'Rio:Help31'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help31'..msg.sender_user_id_)
if SARWTeam == 'msg' then
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help31'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help3', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevRio:set(SARW..'Rio:Help41'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help41'..msg.sender_user_id_)
if SARWTeam == 'msg' then
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help41'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help4', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
DevRio:set(SARW..'Rio:Help51'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help51'..msg.sender_user_id_)
if SARWTeam == 'msg' then
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help51'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help5', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevRio:set(SARW..'Rio:Help61'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local SARWTeam =  DevRio:get(SARW..'Rio:Help61'..msg.sender_user_id_)
if SARWTeam == 'msg' then
Dev_Rio(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevRio:del(SARW..'Rio:Help61'..msg.sender_user_id_)
DevRio:set(SARW..'Rio:Help6', text)
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
local Leave = DevRio:get(SARW..'Rio:Leave')
local Text = [[
...
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:del(SARW.."Rio:WhyTube"..msg.chat_id_) 
return false  
end
if text == 'تعطيل اليوتيوب' and Manager(msg) then  
local SARWTeam = '᥀︙اهلا عزيزي ↫ '..RioRank(msg)..' \n᥀︙تم تعطيل اليوتيوب بنجاح'
riomoned(msg.chat_id_, msg.sender_user_id_, msg.id_, SARWTeam, 14, string.len(msg.sender_user_id_))
DevRio:set(SARW.."Rio:WhyTube"..msg.chat_id_,true) 
return false  
end 
if text and text:match('^بحث (.*)$') and not DevRio:get(SARW.."Rio:WhyTube"..msg.chat_id_) and ChCheck(msg) then            
local Text = text:match('^بحث (.*)$') 
local msg_id = msg.id_/2097152/0.5 
https.request('https://ccccxcc.ml/SARW/Yt.php?Put=Search&TokenBot='..TokenBot..'&ChatId='..msg.chat_id_..'&UserId='..msg.sender_user_id_..'&Text='..URL.escape(Text)..'&MsgId='..msg_id..'&Name='..DirName)
end
--     Source SARW     --
if SecondSudo(msg) then
...
local Users = DevRio:smembers(SARW.."User_Bot")
local Groups = DevRio:smembers(SARW..'Chek:Groups')
local Sudos = DevRio:smembers(SARW.."Sudo:User")
if DevRio:get(SARW..'Name:Bot') then
DevRio:set(SARW..'Rio:NameBot',(DevRio:get(SARW..'Name:Bot') or 'سرو'))
end
for i = 1, #Users do
local id = Users[i]
if id:match("^(%d+)") then
DevRio:sadd(SARW..'Rio:Users',Users[i]) 
end
end
for i = 1, #Sudos do
DevRio:sadd(SARW..'Rio:SudoBot:',Sudos[i]) 
end
for i = 1, #Groups do
DevRio:sadd(SARW..'Rio:Groups',Groups[i]) 
if DevRio:get(SARW.."Private:Group:Link"..Groups[i]) then
DevRio:set(SARW.."Rio:Groups:Links"..Groups[i],DevRio:get(SARW.."Private:Group:Link"..Groups[i]))
end
if DevRio:get(SARW.."Get:Welcome:Group"..Groups[i]) then
DevRio:set(SARW..'Rio:Groups:Welcomes'..Groups[i],DevRio:get(SARW.."Get:Welcome:Group"..Groups[i]))
end
local list2 = DevRio:smembers(SARW..'Constructor'..Groups[i])
for k,v in pairs(list2) do
DevRio:sadd(SARW.."Rio:Constructor:"..Groups[i], v)
end
local list3 = DevRio:smembers(SARW..'BasicConstructor'..Groups[i])
for k,v in pairs(list3) do
DevRio:sadd(SARW.."Rio:BasicConstructor:"..Groups[i], v)
end
local list4 = DevRio:smembers(SARW..'Manager'..Groups[i])
for k,v in pairs(list4) do
DevRio:sadd(SARW.."Rio:Managers:"..Groups[i], v)
end
local list5 = DevRio:smembers(SARW..'Mod:User'..Groups[i])
for k,v in pairs(list5) do
DevRio:sadd(SARW.."Rio:Admins:"..Groups[i], v)
end
local list6 = DevRio:smembers(SARW..'Special:User'..Groups[i])
for k,v in pairs(list6) do
DevRio:sadd(SARW.."Rio:VipMem:"..Groups[i], v)
end
DevRio:set(SARW.."Rio:Lock:Bots"..Groups[i],"del") DevRio:hset(SARW.."Rio:Spam:Group:User"..Groups[i] ,"Spam:User","keed") 
...
for i,Lock in pairs(LockList) do
DevRio:set(SARW..Lock..Groups[i],true)
end
end
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/TeamSARW/SARWFiles/master/getfile.json")
if res == 200 then
...
local GetJson, Res = https.request("https://raw.githubusercontent.com/TeamSARW/SARWFiles/master/SARWFiles/"..FileName)
if Res == 200 then
...
DevRio:del(SARW..'Rio:Text:BotWelcome')
DevRio:del(SARW..'Rio:Photo:BotWelcome')
return false
end 
...
DevRio:del(SARW..'Rio:Lock:BotWelcome')
return false
end 
...
DevRio:set(SARW..'Rio:Lock:BotWelcome',true)
return false
end 
...
DevRio:del(SARW..'Rio:Text:BotWelcome')
DevRio:del(SARW..'Rio:Photo:BotWelcome')
DevRio:set(SARW.."Rio:Set:BotWelcome"..msg.sender_user_id_,"Text") 
return false
end 
if text and DevRio:get(SARW.."Rio:Set:BotWelcome"..msg.sender_user_id_) == 'Text' then 
if text and text:match("^الغاء$") then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر", 1, "md") 
DevRio:del(SARW.."Rio:Set:BotWelcome"..msg.sender_user_id_)   
return false
end 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ النص ارسل لي صورة الترحيب\n᥀︙ارسل ↫ الغاء لحفظ النص فقط", 1, 'md')   
DevRio:set(SARW.."Rio:Text:BotWelcome",text) 
DevRio:set(SARW.."Rio:Set:BotWelcome"..msg.sender_user_id_,"Photo") 
return false 
end 
if DevRio:get(SARW.."Rio:Set:BotWelcome"..msg.sender_user_id_) == 'Photo' then 
if text and text:match("^الغاء$") then 
...
DevRio:set(SARW.."Rio:Photo:BotWelcome",msg.content_.photo_.sizes_[1].photo_.persistent_id_)
...
DevRio:setex(SARW.."Rio:DevText"..msg.chat_id_..":" .. msg.sender_user_id_, 300, true)
end end
...
DevRio:del(SARW.."DevText")
end end
--     Source SARW     --
if DevRio:get(SARW.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Rio(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر", 1, "md") 
DevRio:del(SARW.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevRio:del(SARW.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local texxt = string.match(text, "(.*)") 
DevRio:set(SARW..'Rio:ChText',texxt)
...
DevRio:setex(SARW.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)   
...
Text = [[
᥀︙[مبرمج السورس](https://t.me/lIHHHl)
]]
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '᥀ مبرمج السورس',url="t.me/lIHHHl"}},
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..TokenBot..'/sendPhoto?chat_id=' .. msg.chat_id_ .. '&photo=https://t.me/lIHHHl&caption=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
return false
...
DevRio:incr(SARW..'Rio:EditMsg'..result.chat_id_..result.sender_user_id_)
local text = result.content_.text_ or result.content_.caption_
local Text = result.content_.text_
if DevRio:get(SARW..'Rio:Lock:EditMsgs'..msg.chat_id_) and not Text and not RioConstructor(result) then
...
local GetMsgPin = DevRio:get(SARW..'Rio:PinnedMsgs'..msg.chat_id_)
if GetMsgPin ~= nil then
if text == GetMsgPin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) if dp.ID == 'Ok' then;DevRio:del(SARW..'Rio:PinnedMsgs'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if GetMsgPin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) DevRio:del(SARW..'Rio:PinnedMsgs'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_=v,user_id_=SARW,status_={ID = "ChatMemberStatusLeft"},},function(e,g) end, nil) 
DevRio:srem(SARW..'Rio:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
DevRio:srem(SARW..'Rio:Groups',v)  
end
